...
class QuantoniumIntro(QMainWindow):
    ...
    start_crossfade = pyqtSignal()  # Signal to start cross-fade transition
    ...
    def update_text(self, elapsed):
        """Show text and prepare for cross-fade"""
        # Title
        if elapsed > 0:
        ...
        # Start cross-fade after text is visible
        if elapsed > 1500 and not self.crossfade_started:
            self.crossfade_started = True
            self.start_crossfade.emit()
        
        # Continue for a bit longer to allow cross-fade
        if elapsed > 3000:
            self.current_phase = "complete"
    
    def start_fadeout(self):
        """Begin fading out the intro window"""
    ...
    def fade_step(self):
        ...
        self.fade_opacity -= 0.02  # Fade over ~1.25 seconds
