...
def analyze_usage():
    ...
    unique_countries = len([c for c in country_stats.keys() if c not in ['Unknown', 'Internal/CDN']])
    ...
    return {
        ...
        'ip_details': [(ip, ip_to_country.get(ip, 'Unknown'), ip_counts.get(ip, 1)) 
                      for ip in unique_ips if ip_to_country.get(ip, 'Unknown') not in ['Internal/CDN']]
    }
