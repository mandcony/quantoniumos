||#!/usr/bin/env python3
""""""
Focused Quantum Simulator Capacity Test

This test determines the exact qubit limit by testing incrementally
and provides detailed memory and performance analysis.
""""""
...
def test_single_qubit_count(n_qubits: int) -> Dict:
    ...
    try:
        print(f" Testing {n_qubits} qubits (2^{n_qubits} = {2**n_qubits:,} states)...")
        ...
        norm_error = abs(1.0 - np.linalg.norm(sim_test.state))
        ...
        result = {
            ...
            'memory_after_ops': memory_after,
            'memory_used': memory_after - memory_before,
            'init_time': init_time,
            'gates_time': gates_time,
            'prob_time': prob_time,
...
def find_maximum_capacity():
    ...
    # Start with known working range
    working_qubits = []
    failed_qubits = []
    ...
    for n in range(1, 11):
        result = test_single_qubit_count(n)
        if result['success']:
            working_qubits.append(result)
        else:
            failed_qubits.append(result)
            break
    ...
    max_qubits = working_qubits[-1]['n_qubits']
    ...
    for result in working_qubits[-5:]:  # Show last 5 results
        n = result['n_qubits']
        states = result['state_size']
        memory = result['memory_used']
        total_time = result['total_time']
        init_time = result['init_time'] * 1000
        gates_time = result['gates_time'] * 1000
        prob_time = result['prob_time'] * 1000

        print(f"{n:6} | {states:10,} | {memory:9.1f} | {total_time:6.3f} | "
              f"{init_time:7.1f} | {gates_time:8.1f} ||| {prob_time:7.1f}")

    # Precision analysis
    precision_errors = [r['norm_error'] for r in working_qubits]
    ...
    if max_error < 1e-14:
        precision_grade = "EXCELLENT (machine precision)"
    elif max_error < 1e-12:
        precision_grade = "VERY GOOD"
    elif max_error < 1e-10:
        precision_grade = "GOOD"
    else:
        precision_grade = "POOR - numerical instability detected"
    ...
    print(f"\n💼 PRACTICAL APPLICATIONS AT {max_qubits} QUBITS:")
    applications = [
        (5, "Quantum error correction codes"),
        (8, "Quantum Fourier Transform demos"),
        (10, "Grover's algorithm on small databases"), (12, "Quantum chemistry simulations (small molecules)"), (15, "Quantum machine learning experiments"), (20, "Advanced quantum algorithms research"), (25, "Professional quantum software development") ] for req_qubits, application in applications: if max_qubits >= req_qubits: print(f" ✅ {application}") else: print(f" ❌ {application} (requires {req_qubits} qubits)") # Memory efficiency analysis last_result = working_qubits[-1] memory_per_state = last_result['memory_used'] * 1024 * 1024 / last_result['state_size']  # bytes per state
    complex128_size = 16  # bytes for complex128
