...
def main():
    ...
    try:
        ...
        # Calculate MSE
        basic_mse = sum(
            (a - b) ** 2 for a, b in zip(test_signal, basic_reconstructed)
        ) / len(test_signal)
        high_level_mse = sum(
            (a - b) ** 2 for a, b in zip(test_signal, high_level_reconstructed)
        ) / len(test_signal)
    ...
    try:
        ...
        print(
            f"✅ Measurement ratios: |0⟩={zero_count/100:.3f}, |1⟩={one_count/100:.3f}"
        )
    ...
    available_components = sum(
        1
        for comp in results["validation_components"].values()
