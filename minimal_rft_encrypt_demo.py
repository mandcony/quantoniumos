...
True RFT to a simple XOR stream cipher style construction.

Algorithm (demonstration only, NOT production crypto):
...
4. Output hex ciphertext with embedded block size.

Decrypt reverses using same deterministic keystream regeneration.

Usage:
    python minimal_rft_encrypt_demo.py "secret pass" "Hello RFT World!"
...
def rft_keystream(index: int, block_len: int, key: bytes) -> bytes:
    ...
    seed = bytearray(key)
    for i in range(8):
        seed[i % len(seed)] ^= (index >> (8*i)) & 0xFF
    vec = [float(b) for b in seed[:block_len]]
    # Pad if needed
    if len(vec) < block_len:
        vec.extend([0.0]*(block_len-len(vec)))
    coeffs = forward_true_rft(vec[:block_len])
    raw = []
    for c in coeffs:
        raw.append(c.real)
        raw.append(c.imag)
    packed = ('|'.join(f"{x:.17g}" for x in raw)).encode('utf-8')
    digest = hashlib.sha512(packed).digest()
    return digest[:block_len]

def encrypt(passphrase: str, plaintext: str, block_size: int = 32) -> str:
    key = derive_key(passphrase)
    data = plaintext.encode('utf-8')
    out = []
    for bi, i in enumerate(range(0, len(data), block_size)):
        block = data[i:i+block_size]
        ks = rft_keystream(bi, len(block), key)
        out.append(bytes(a ^ b for a,b in zip(block, ks)))
...
def decrypt(passphrase: str, ciphertext: str) -> str:
    ...
    meta, hexdata = ciphertext.split('-',1)
    block_size = int(meta[3:5])
    key = derive_key(passphrase)
    ...
    for bi, i in enumerate(range(0, len(data), block_size)):
        block = data[i:i+block_size]
        ks = rft_keystream(bi, len(block), key)
        out_blocks.append(bytes(a ^ b for a,b in zip(block, ks)))
