...
// ===== Minimal SHA-256 (big-endian) for key derivation (audited libs recommended in prod) =====
class SHA256 {
private:
...
public:
    static std::vector<uint8_t> hash(const std::vector<uint8_t>& data){
        ...
        std::vector<uint8_t> m; m.reserve(data.size()+72);
...
// ===== Enhanced RFT Feistel (single-Feistel-over-whole-buffer, as in your current design) =====
...
private:
    ...
    static constexpr size_t  F_MIN  = 16;   // operate at least on 16 bytes in ARX path

    // AES S-box (public domain tables)
    ...
    // Small, bijective byte-permutation family (16 options for 16 positions).
    // For lengths != 16 we map via modulo (no collisions if len==16).
    ...
    // Cached schedule (NOT thread-safe by design)
    ...
    mutable std::vector<uint32_t> cached_rc32; // round constants for extra key mixing
    mutable std::vector<uint8_t> last_key;

    // ---- GF(2^8) helpers for AES-like MDS (branchless) ----
    static inline uint8_t xtime(uint8_t x){
        uint8_t m = static_cast<uint8_t>(-(x >> 7)); // 0xFF if high bit set
    ...
    // MixColumns on 16 bytes (4 columns of 4 bytes): strong linear diffusion.
    ...
    // ----- Key schedule (domain-separated; cached) -----
    bool derive_keys_cached(const std::vector<uint8_t>& master_key) const {
        ...
        auto derive_subkey = [&](const std::string& domain, uint8_t ctr)->std::vector<uint8_t>{
            std::vector<uint8_t> in; in.reserve(master_key.size()+domain.size()+2);
            ...
            // 3 extra iterations for simple stretching/domain hardening
            for (int i=0;i<3;++i){
                std::vector<uint8_t> x; x.reserve(h.size()+1);
        ...
        for (size_t r=0; r<ROUNDS; ++r){
            ...
            // rotation amount from rk[16..23] (legacy for compatibility)
            uint64_t rot_seed=0;
            ...
            // stronger perm selection (0..15) but from different bytes than before
            uint8_t pidx = static_cast<uint8_t>((rk[28] ^ rk[5] ^ rk[11] ^ rk[3]) & 0x0F);
            cached_perm_idx.push_back(pidx);

            // round constant for additional key mixing inside F
    ...
    // ----- F-function (enhanced with stronger diffusion) -----
                                         ...
                                         uint8_t* out) const
    {
        ...
        // --- cheap PHI-ish positional stir (integer-only) ---
        for (size_t i=0;i<L;++i){
            uint32_t t = static_cast<uint32_t>((i+1) * 2654435761u); // golden/Knuth
            state[i] ^= static_cast<uint8_t>(t);
        }

        // --- key inject #1 (XOR) ---
        for (size_t i=0;i<L;++i)
            state[i] ^= round_key[i % round_key.size()];

        // --- ARX on first 16 bytes, now with 3 independent rotates ---
        if (L >= 16){
            uint32_t w0,w1,w2,w3;
            std::memcpy(&w0, state+ 0, 4);
            std::memcpy(&w1, state+ 4, 4);
            std::memcpy(&w2, state+ 8, 4);
            std::memcpy(&w3, state+12, 4);

            // derive two more rotates from already-mixed bytes (cheap churn)
            int rA = rot_primary;
            ...
            // 4 quarter-rounds total (2× the previous), using all three rotates
            qr(rA, rB);
            ...
            std::memcpy(state+ 0, &w0, 4);
            std::memcpy(state+ 4, &w1, 4);
            std::memcpy(state+ 8, &w2, 4);
            std::memcpy(state+12, &w3, 4);
        }

        // --- S-box layer ---
        for (size_t i=0;i<L;++i) state[i] = SBOX[state[i]];

        // --- strong linear diffusion over first 16 bytes ---
        if (L >= 16){
            mixcolumns_4x4(state); // columns
            // rotate rows (ShiftRows-like) to avoid static alignment
            uint8_t t;
            // row1 shift 1
...
public:
                                 ...
                                 const std::vector<uint8_t>& key) const
    {
        ...
        for (size_t r=0; r<ROUNDS; ++r){
            enhanced_f_function_fast(R.data(), half, cached_round_keys[r],
                                     cached_rotations[r], cached_perm_idx[r],
                                     Fout.data());
            xor_bytes(L.data(), Fout.data(), half);
            if (r < ROUNDS-1) L.swap(R);
        }

        post_whiten(L.data(), half);
        post_whiten(R.data(), half);

        std::vector<uint8_t> C; C.reserve(plaintext.size());
                                 ...
                                 const std::vector<uint8_t>& key) const
    {
        ...
        // reverse pre-whitening (BUG FIX: check pre key size, not post)
        pre_whiten(L.data(), half);
        pre_whiten(R.data(), half);

        std::vector<uint8_t> P; P.reserve(ciphertext.size());
...
extern "C" {
    EnhancedRFTCrypto* create_enhanced_rft(){ return new EnhancedRFTCrypto(); }
    void destroy_enhanced_rft(EnhancedRFTCrypto* c){ delete c; }

    int encrypt_enhanced_rft(EnhancedRFTCrypto* c,
                             ...
                             uint8_t* ct, size_t* ct_len)
    {
        try{
            ...
            if (*ct_len < out.size()){ *ct_len = out.size(); return -1; }
    ...
    int decrypt_enhanced_rft(EnhancedRFTCrypto* c,
                             ...
                             uint8_t* pt, size_t* pt_len)
    {
        try{
            ...
            if (*pt_len < out.size()){ *pt_len = out.size(); return -1; }
