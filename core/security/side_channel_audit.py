...
class CppSourceFile:
    """Class representing a C++ source file for analysis"""
...
class RuntimeVerifier:
    ...
    def detect_timing_leak(self, timing_results: Dict) -> Dict:
        ...
        # Determine if there's a likely timing leak
        significant_tests = sum(1 for test in t_tests if test['significant'])
        leak_likelihood = significant_tests / len(t_tests) if t_tests else 0
        
        leak_detected = leak_likelihood > 0.3  # More than 30% of tests show significant differences
        ...
        return {
            ...
            'coefficient_variation': cvs,
            'detailed_tests': t_tests
        }
    
    def plot_timing_distributions(self, timing_results: Dict, output_file: str = None):
        """
        Plot timing distributions to visualize potential leaks
        ...
        """
        if 'error' in timing_results:
            print(f"Cannot plot: {timing_results['error']}")
            return
        ...
        # Plot histogram for each input variation
        for i, data in enumerate(timing_data):
            trials = data['trials']
            plt.hist(trials, bins=30, alpha=0.5, label=f"Input {i+1}: {data['input_prefix']}")
        
        plt.title(f"Timing Distribution: {function_name}")
...
class CacheSideChannelAnalyzer:
    """Analyzer for cache-based side-channel vulnerabilities"""
    ...
    def flush_cache(self):
        """Flush the CPU cache before measurement"""
        # This is a simplistic approach - in a real implementation this would
        # use more sophisticated techniques like CLFLUSH or large memory traversal
        
        # Allocate a large array and access it to flush cache
        array_size = 16 * 1024 * 1024  # 16 MB
        array = bytearray(array_size)
        
        # Access the array to flush cache
        for i in range(0, array_size, 64):  # 64 bytes is a common cache line size
            array[i] = 1
    
    def measure_cache_access_pattern(self, function_name: str, input_variations: List[bytes],
                                  num_trials: int = 100) -> Dict:
        """
        Measure cache access patterns to detect cache-based side channels
        
        This is a simplified simulation - real cache side-channel detection
        would use hardware performance counters or techniques like Flush+Reload
        ...
        Returns:
            Dictionary of cache measurement results
        """
        if not self.lib:
            return {"error": "Library not loaded"}
        ...
        try:
            ...
            for input_data in input_variations:
                ...
                for _ in range(num_trials):
                    ...
                    # Measure cache misses (simulated here)
                    # In a real implementation, we would use perf events or similar
                    start = time.perf_counter_ns()
                    func(input_buffer, input_size)
                    end = time.perf_counter_ns()
                    
                    # We're using time as a proxy for cache behavior here
    ...
    def detect_cache_leak(self, cache_results: Dict) -> Dict:
        ...
        # Determine if there's a likely cache leak
        significant_tests = sum(1 for test in t_tests if test['significant'])
        leak_likelihood = significant_tests / len(t_tests) if t_tests else 0
...
class ConstantTimeAudit:
    """Comprehensive constant-time audit for C++ code"""
    
    def __init__(self, cpp_directory: str, output_directory: str = "audit_results"):
        self.cpp_directory = cpp_directory
        ...
        self.static_analyzer = StaticAnalyzer(cpp_directory)
        
        if self.lib_path:
            self.runtime_verifier = RuntimeVerifier(self.lib_path)
            self.cache_analyzer = CacheSideChannelAnalyzer(self.lib_path)
        else:
            self.runtime_verifier = None
            self.cache_analyzer = None
    
    def _find_library_path(self) -> Optional[str]:
        """Find the path to the compiled library"""
        # Look for .so, .dll, or .dylib files
        ...
        # Also check in standard directories like 'build', 'lib', 'bin'
        for dir_name in ['build', 'lib', 'bin', 'out', 'output']:
            check_dir = os.path.join(self.cpp_directory, dir_name)
    ...
    def run_static_analysis(self) -> Dict:
        """Run static analysis"""
        print("Running static analysis for side-channel vulnerabilities...")
        
        # Scan the directory for C++ files
        self.static_analyzer.scan_directory()
        
        # Analyze the files
        static_results = self.static_analyzer.analyze_files()
        
        # Save results
        self._save_json(static_results, "static_analysis_results.json")
        
        return static_results
    
    def run_runtime_verification(self, functions_to_test: Dict[str, List[bytes]]) -> Dict:
        """
        Run runtime verification
        
        Args:
            functions_to_test: Dictionary mapping function names to lists of input variations
        
        Returns:
            Dictionary of verification results
        """
        if not self.runtime_verifier:
            return {"error": "No library found for runtime verification"}
        
        print("Running runtime verification for constant-time execution...")
        
        runtime_results = {}
        
        for function_name, input_variations in functions_to_test.items():
            print(f"Testing function: {function_name}")
            
            # Measure timing
            timing_results = self.runtime_verifier.time_function(
                function_name, input_variations, num_trials=1000
            )
            
            # Detect timing leaks
            leak_results = self.runtime_verifier.detect_timing_leak(timing_results)
            
            # Plot timing distributions
            plot_file = os.path.join(self.output_directory, f"timing_{function_name}.png")
            self.runtime_verifier.plot_timing_distributions(timing_results, plot_file)
            
            runtime_results[function_name] = {
                'timing_results': timing_results,
                'leak_analysis': leak_results,
                'plot_file': plot_file
            }
        
        # Save results
        self._save_json(runtime_results, "runtime_verification_results.json")
        
        return runtime_results
    
    def run_cache_analysis(self, functions_to_test: Dict[str, List[bytes]]) -> Dict:
        """
        Run cache side-channel analysis
        
        Args:
            functions_to_test: Dictionary mapping function names to lists of input variations
        
        Returns:
            Dictionary of cache analysis results
        """
        if not self.cache_analyzer:
            return {"error": "No library found for cache analysis"}
        
        print("Running cache side-channel analysis...")
        
        cache_results = {}
        
        for function_name, input_variations in functions_to_test.items():
            print(f"Analyzing cache patterns for function: {function_name}")
            
            # Measure cache patterns
            measurements = self.cache_analyzer.measure_cache_access_pattern(
                function_name, input_variations, num_trials=100
            )
            
            # Detect cache leaks
            leak_results = self.cache_analyzer.detect_cache_leak(measurements)
            
            cache_results[function_name] = {
                'cache_measurements': measurements,
                'leak_analysis': leak_results
            }
        
        # Save results
        self._save_json(cache_results, "cache_analysis_results.json")
        
        return cache_results
    
    def run_comprehensive_audit(self, functions_to_test: Dict[str, List[bytes]]) -> Dict:
        """
        Run comprehensive constant-time audit
        ...
        print(f"Starting comprehensive constant-time audit at {timestamp}")
        
        # Run all analyses
        static_results = self.run_static_analysis()
        runtime_results = self.run_runtime_verification(functions_to_test)
        cache_results = self.run_cache_analysis(functions_to_test)
        
        # Combine results
        comprehensive_results = {
            'timestamp': timestamp,
            'cpp_directory': self.cpp_directory,
            'lib_path': self.lib_path,
            'static_analysis': static_results,
            'runtime_verification': runtime_results,
            'cache_analysis': cache_results
        }
        
        # Save comprehensive results
        self._save_json(comprehensive_results, "comprehensive_audit_results.json")
    ...
    def _save_json(self, data: Dict, filename: str):
        ...
        data_copy = self._prepare_for_json(data)
        
        with open(file_path, 'w') as f:
            json.dump(data_copy, f, indent=2)
    
    def _prepare_for_json(self, data):
        """Prepare data for JSON serialization by removing non-serializable elements"""
        if isinstance(data, dict):
            return {k: self._prepare_for_json(v) for k, v in data.items() 
                   if k not in ['lib', 'trials']}
    ...
    def _generate_report(self, results: Dict):
        ...
        report_file = os.path.join(self.output_directory, f"audit_report_{timestamp}.html")
        
        static_results = results.get('static_analysis', {})
        runtime_results = results.get('runtime_verification', {})
        cache_results = results.get('cache_analysis', {})
        
        # Count violations and leaks
        total_static_violations = static_results.get('total_violations', 0)
        
        runtime_leaks = sum(1 for func in runtime_results.values() 
                          if isinstance(func, dict) and func.get('leak_analysis', {}).get('leak_detected'))
        
        cache_leaks = sum(1 for func in cache_results.values() 
                        if isinstance(func, dict) and func.get('leak_analysis', {}).get('leak_detected'))
        ...
        with open(report_file, 'w') as f:
            ...
            <html>
            <head>
                <title>Constant-Time Audit Report: {timestamp}</title>
                <style>
            ...
            <body>
                <h1>QuantoniumOS Constant-Time Audit Report</h1>
                <p>Generated on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}</p>
                
                <div class="summary">
                    <h2>Executive Summary</h2>
                    <p><b>C++ Directory:</b> {results['cpp_directory']}</p>
                    <p><b>Library:</b> {results['lib_path'] or 'Not found'}</p>
                    <p><b>Static Analysis Violations:</b> 
                       <span class="{'pass' if total_static_violations == 0 else 'fail'}">
                       {total_static_violations}
                       </span>
                    </p>
                    <p><b>Runtime Timing Leaks:</b> 
                       <span class="{'pass' if runtime_leaks == 0 else 'fail'}">
                       {runtime_leaks}
                       </span>
                    </p>
                    <p><b>Cache Side-Channel Leaks:</b> 
                       <span class="{'pass' if cache_leaks == 0 else 'fail'}">
                       {cache_leaks}
                       </span>
                    </p>
                    <p><b>Overall Assessment:</b> 
                       <span class="{'pass' if total_static_violations == 0 and runtime_leaks == 0 and cache_leaks == 0 else 'fail'}">
                       {'PASS' if total_static_violations == 0 and runtime_leaks == 0 and cache_leaks == 0 else 'FAIL'}
                       </span>
                    </p>
                </div>
            """)
            ...
            f.write("""
                <h2>1. Static Analysis Results</h2>
            """)
            
            if isinstance(static_results, dict) and 'total_files' in static_results:
                f.write(f"""
                <p>Analyzed {static_results['total_files']} C++ files.</p>
                <p>Found {static_results['files_with_violations']} files with violations.</p>
                <p>Total security-critical functions: {static_results['security_critical_functions']}</p>
                ...
                for vtype, count in static_results.get('violations_by_type', {}).items():
                    f.write(f"""
                    <tr>
                        <td>{vtype}</td>
                        <td>{count}</td>
                ...
                if static_results.get('files_with_violations', 0) > 0:
                    f.write("""
                    <h3>Files with Violations:</h3>
                    """)
                    
                    for file_path, file_results in static_results.get('violations_by_file', {}).items():
                        f.write(f"""
                        <h4>{file_path}</h4>
                        <p>Total violations: {file_results['total_violations']}</p>
                        """)
                        
                        if file_results.get('branching_on_secret'):
                            f.write("""
                            <h5>Branching on Secret Data:</h5>
                            <table>
                                <tr>
                                    <th>Function</th>
                                    <th>Type</th>
                            ...
                            for violation in file_results['branching_on_secret']:
                                f.write(f"""
                                <tr>
                                    <td>{violation['function']}</td>
                                    <td>{violation['type']}</td>
                                    <td><pre>{violation['snippet']}</pre></td>
                                </tr>
                                """)
                            
                            f.write("</table>")
                        
                        if file_results.get('memory_access_on_secret'):
                            f.write("""
                            <h5>Secret-Dependent Memory Access:</h5>
                            <table>
                                <tr>
                                    <th>Function</th>
                                    <th>Type</th>
                            ...
                            for violation in file_results['memory_access_on_secret']:
                                f.write(f"""
                                <tr>
                                    <td>{violation['function']}</td>
                                    <td>{violation['type']}</td>
                ...
                f.write("<p>No static analysis results available.</p>")
            
            # Runtime Verification Section
            f.write("""
                <h2>2. Runtime Verification Results</h2>
            """)
            
            if isinstance(runtime_results, dict) and not runtime_results.get('error'):
                for function_name, func_results in runtime_results.items():
                    if not isinstance(func_results, dict):
                        continue
                    
                    leak_analysis = func_results.get('leak_analysis', {})
                    leak_detected = leak_analysis.get('leak_detected')
                    
                    f.write(f"""
                    <h3>Function: {function_name}</h3>
                    <p><b>Timing Leak Detected:</b> 
                       <span class="{'fail' if leak_detected else 'pass'}">
                       {leak_detected}
                       </span>
                    </p>
                    """)
                    ...
                    if leak_detected:
                        f.write(f"""
                        <p><b>Leak Likelihood:</b> {leak_analysis.get('leak_likelihood', 0):.2f}</p>
                        <p><b>Significant Test Results:</b> {leak_analysis.get('significant_tests', 0)} / {leak_analysis.get('total_tests', 0)}</p>
                        """)
                    
                    # Show timing distribution plot
                    plot_file = func_results.get('plot_file')
                    if plot_file:
                        plot_filename = os.path.basename(plot_file)
                        f.write(f"""
                        <h4>Timing Distribution:</h4>
                        <img src="{plot_filename}" alt="Timing Distribution">
                        """)
            else:
                error_msg = runtime_results.get('error', "Unknown error")
                f.write(f"<p>Runtime verification could not be performed: {error_msg}</p>")
            
            # Cache Analysis Section
            f.write("""
                <h2>3. Cache Side-Channel Analysis</h2>
            """)
            
            if isinstance(cache_results, dict) and not cache_results.get('error'):
                for function_name, func_results in cache_results.items():
                    if not isinstance(func_results, dict):
                        continue
                ...
                error_msg = cache_results.get('error', "Unknown error")
                f.write(f"<p>Cache analysis could not be performed: {error_msg}</p>")
            ...
            if total_static_violations > 0:
                f.write("""
                <li>Fix static violations by implementing constant-time alternatives:
                    <ul>
                        <li>Replace secret-dependent branching with constant-time conditional operations</li>
                        <li>Replace variable-time functions like strcmp with constant-time equivalents</li>
                        <li>Ensure memory access patterns don't depend on secret values</li>
                    </ul>
                </li>
                """)
