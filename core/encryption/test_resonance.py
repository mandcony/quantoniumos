...
def test_avalanche_effect(num_tests: int = 1000) -> bool:
    """Test the avalanche effect by changing single bits"""
    print("\nTesting avalanche effect...")
    print(f"Running {num_tests} tests...")
    
    def count_bit_differences(bytes1: bytes, bytes2: bytes) -> Tuple[int, int]:
        """Count how many bits differ between two byte sequences"""
        diff_bits = 0
        total_bits = min(len(bytes1), len(bytes2)) * 8
    ...
    key = "test_key_123"
    base_input = "QuantoniumOS Resonance Test Data" * 4
    ...
    for test in range(num_tests):
        ...
        # Encrypt original
        cipher1 = resonance_encrypt(base_input, key)
        
        # Modify one bit in input
        modified = list(base_input)
        char_pos = test % len(modified)
        char = ord(modified[char_pos])
        bit_pos = (test // len(modified)) % 8
        char ^= (1 << bit_pos)
        modified[char_pos] = chr(char)
        
        # Encrypt modified
        cipher2 = resonance_encrypt(''.join(modified), key)
