...
def generate_keystream(seed: bytes, length: int, max_chunk_size: int = 1024*1024) -> bytes:
    ...
    for i in range(num_chunks):
        ...
        # Generate chunk using fixed salt
        chunk = hashlib.pbkdf2_hmac(
            'sha256',
            chunk_seed,
            stream_salt,  # Fixed salt based on seed
            100,  # Iterations
            this_chunk_size
        )
        keystream.extend(chunk)
        
        # Progress for large streams
        if length > 1024*1024:  # 1MB
            print(f"Generating keystream... {len(keystream)/length*100:.1f}%")
            
    return bytes(keystream)

def fixed_resonance_encrypt(plaintext: Union[str, bytes], key: str) -> bytes:
    """
    Fixed version of resonance encryption with safety checks
    """
    ...
    # Enforce size limits
    if len(data) > 1024 * 1024 * 50:  # 50MB max
        raise ValueError("Input data too large")
        
    # Generate key hash and signature
    key_hash = hashlib.sha256(key.encode()).digest()
    signature = key_hash[:8]
    token = secrets.token_bytes(32)
    
    # Generate keystream safely
    try:
        keystream = generate_keystream(key_hash + token, len(data))
    except Exception as e:
        raise RuntimeError(f"Keystream generation failed: {str(e)}")
    
    # Encrypt data with progress tracking
    result = bytearray(len(data))
    for i in range(len(data)):
        # XOR with keystream
        result[i] = data[i] ^ keystream[i]
        # Rotate bits (with safety check)
        ...
        if len(data) > 1024*1024 and i % (1024*1024) == 0:  # Every 1MB
            print(f"Encrypting... {i/len(data)*100:.1f}%")
            
    return signature + token + bytes(result)

def fixed_resonance_decrypt(encrypted_data: bytes, key: str) -> Union[str, bytes]:
    """
    Fixed version of resonance decryption with safety checks
    """
    # Size validation
    if len(encrypted_data) < 41:  # 8 bytes sig + 32 bytes token + at least 1 byte data
    ...
    # Decrypt data with progress tracking
    result = bytearray(len(data))
    for i in range(len(data)):
        ...
        # Progress tracking for large inputs
        if len(data) > 1024*1024 and i % (1024*1024) == 0:  # Every 1MB
            print(f"Decrypting... {i/len(data)*100:.1f}%")
    
    # Convert back to string or return bytes
    try:
