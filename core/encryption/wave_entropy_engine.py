...
class WaveformEntropyEngine:
    ...
    def generate_entropy_bytes(self, num_bytes: int) -> bytes:
        """
        Generates entropy bytes using the waveform's current state.
        
        Args:
            num_bytes: Number of bytes to generate
            
        Returns:
            Bytes object containing the generated entropy
        """
        result = bytearray()
        
        while len(result) < num_bytes:
            # Extract entropy from current waveform state
            byte_values = []
            for w in self.waveform:
                # Map amplitude (0-1) to byte value (0-255)
                byte_val = int(w.amplitude * 256) % 256
                # Use phase to introduce additional variation
                phase_factor = int(w.phase * 128 / math.pi) % 256
                # Combine amplitude and phase factors
                combined = (byte_val + phase_factor) % 256
                byte_values.append(combined)
            
            # Apply additional mixing for increased entropy
            mixed = bytearray()
            for i in range(len(byte_values)):
                val = byte_values[i]
                if i > 0:
                    val = (val + byte_values[i-1]) % 256
                mixed.append(val)
            
            result.extend(mixed)
            
            # Mutate the waveform for the next iteration
            self.mutate_waveform()
            self.dynamic_feedback()
        
        return bytes(result[:num_bytes])  # Truncate to requested length
    ...
    def run_engine(self, iterations=100, target_entropy=0.8):
        ...
        and records the entropy and mutation rate.
        """
        results = []
        for i in range(iterations):
            ...
            logger.debug(f"Iteration {i+1}: Entropy = {entropy:.4f}, Mutation Rate = {self.mutation_rate:.4f}")
        
        return results

# Create a global engine instance for use across the application
global_entropy_engine = WaveformEntropyEngine()

def get_quantum_random_bytes(num_bytes: int) -> bytes:
    """
    Public function to get quantum-inspired random bytes from the global engine.
    """
