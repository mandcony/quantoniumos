    ...
    try:
        ...
        if waveform:
            result["amplitude"] = round(
                sum(abs(x) for x in waveform) / len(waveform), 4
            )
...
def perform_irft(rft_result: Dict[str, Any]) -> List[float]:
    ...
    try:
        ...
        for i in range(min(10, length)):  # Up to 10 frequencies or signal length
            ...
            if freq_name in rft_result or (
                re_name in rft_result and im_name in rft_result
            ):
                freq = i / length
                ...
                if re_name in rft_result and im_name in rft_result:
                    complex_val = complex(
                        float(rft_result[re_name]), float(rft_result[im_name])
                    )
        ...
        if not freq_components:
            logger.warning(
                "No frequency components found in RFT result, creating approximation"
            )
        ...
        logger.info(
            f"IRFT completed, reconstructed waveform of length {len(reconstructed)}"
        )
...
def perform_rft_list(signal: List[float]) -> List[Tuple[float, complex]]:
    ...
    significant_components = [
        (freq, comp)
        for freq, comp in full_spectrum
    ...
    if kept_energy > 0:
        ...
        significant_components = [
            (freq, comp * normalization_factor) for freq, comp in significant_components
        ]
