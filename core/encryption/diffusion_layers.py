...
def test_avalanche_effect(data: bytes, rounds: int = 3, bits_to_flip: int = 1) -> Dict[str, Any]:
    ...
    for bit_pos in range(min(total_bits, 128)):  # Limit to first 128 bits for large data
...
def plot_avalanche_distribution(data: bytes, rounds: list = [1, 2, 3, 4], output_file: str = None):
    ...
    for r in rounds:
        ...
        # Plot histogram
        plt.hist(bit_changes, bins=20, alpha=0.5, label=f"{r} rounds (avg: {result['average_bit_change_percent']:.2f}%)")
