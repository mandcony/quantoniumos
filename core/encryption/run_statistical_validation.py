...
def aggregate_results(results: List[Dict]) -> Dict:
    ...
    for test_name in results[0]["nist_tests"].keys():
        passes = sum(1 for r in results if r["nist_tests"][test_name].get("pass", False))
...
def write_summary_report(f, aggregate: Dict, total_time: float):
    ...
    for test_name, results in aggregate["nist_tests"].items():
        ...
        f.write(f" p-value range: [{results['min_p_value']:.4f}, {results['max_p_value']:.4f}]\n")
...
if __name__ == "__main__":
    ...
    results = run_long_statistical_validation(
        num_iterations=5,
        sample_size_mb=10,
        export_dir="statistical_validation"
    )
