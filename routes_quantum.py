    ...
    try:
        ...
        if data.get('format') == 'csv' and 'perturbation_results' in result:
            ...
            for entry in perturbation_data:
                csv_lines.append(f"{entry['id']},{entry['perturbation']:.6f},{entry['fidelity']:.6f},{entry['resonance']:.6f}")
            ...
            return Response(
                csv_content,
                mimetype="text/csv",
                headers={"Content-disposition": "attachment; filename=perturbation_results.csv"}
            )
