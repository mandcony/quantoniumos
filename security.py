...
from utils.security_logger import (SecurityEventType, SecurityOutcome,
                                   log_rate_limit_exceeded, log_security_event,
...
def get_cors_origins():
    ...
    if not origins_env:
        ...
        if os.environ.get("LOCAL_DEVELOPMENT") == "true":
            default_origins.extend(["https://localhost:5000", "https://localhost:3000"])
...
def cors_check():
    ...
    static_resources = request.path.startswith("/static/") or request.path.startswith(
        "/wave_ui/"
    )
...
def configure_security(app: Flask):
    ...
    from redis_config import REDIS_AVAILABLE, redis_cluster
    ...
    if REDIS_AVAILABLE and redis_cluster.redis_client:
        ...
        if redis_url:
            ...
            logger.warning(
                "Redis not available. Using memory storage for rate limits - will not work across multiple instances."
            )
            storage_uri = "memory://"
            storage_options = {}
    ...
    limiter = Limiter(
        app=app, key_func=get_remote_address, default_limits=default_limits
    )
    ...
    encryption_limit = limiter.shared_limit(
        "60 per minute", scope="encryption_endpoints"
    )
    ...
    def set_additional_headers(response):
        ...
        api_endpoints = request.path.startswith("/api/")
        static_resources = request.path.startswith(
            "/static/"
        ) or request.path.startswith("/wave_ui/")
