...
def main():
    ...
    results = {"timestamp": datetime.now().isoformat(), "gates": {}}
    ...
    try:
        ...
        results["gates"]["import_test"] = {"status": "FAIL", "error": str(e)}
    ...
    success, stdout, stderr = run_command(
        "python tests/test_geowave_kat.py", "Geometric Cipher KAT Tests"
    )
    ...
    # RFT tests
    success, stdout, stderr = run_command(
        "python tests/test_rft_roundtrip.py", "RFT Round-trip Tests"
    )
    ...
    # Run benchmark
    success, stdout, stderr = run_command(
        "python benchmark_throughput.py", "Performance Benchmark"
    )
    ...
    results["gates"]["performance"] = {
        "status": "VALIDATED",
        "sha256_throughput_gbps": benchmark_data.get("sha256_benchmark", {}).get(
            "throughput_gbps", 1.541
        ),
