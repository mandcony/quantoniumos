...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Quantonium OS Command Line Interface"
    )
    ...
    # RFT command
    rft_parser = subparsers.add_parser("rft", help="Perform Resonance Fourier Transform")
    rft_parser.add_argument("waveform", help="Comma-separated waveform values (e.g. '0.1,0.5,0.9')")
    
    # Entropy command
    entropy_parser = subparsers.add_parser("entropy", help="Generate entropy")
    entropy_parser.add_argument("--amount", type=int, default=32, help="Amount of entropy to generate (1-1024)")
    
    # Container command
    container_parser = subparsers.add_parser("unlock", help="Unlock a symbolic container")
    container_parser.add_argument("waveform", help="Comma-separated waveform values (e.g. '0.2,0.7,0.3')")
