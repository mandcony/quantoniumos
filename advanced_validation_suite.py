...
class AdvancedValidationSuite:
                ...
                for noise_level in noise_levels:
                    ...
                    for trial in range(trials_per_level):
                        ...
                        try:
                            ...
                            # If reconstruction fails, that's also important data
                            errors_at_level.append(float('inf'))
                    
                    if errors_at_level:
                        avg_error = np.mean([e for e in errors_at_level if np.isfinite(e)])
                        if np.isfinite(avg_error):
                            size_lipschitz.append(avg_error)
                            error_pairs.append((noise_level, avg_error))
                
                if size_lipschitz:
                    # Estimate Lipschitz constant (max slope)
                    lipschitz_estimate = np.max(size_lipschitz) if size_lipschitz else float('inf')
                    lipschitz_constants.append(lipschitz_estimate)
                    
                    # Find noise tolerance (where error becomes significant)
                    tolerance_threshold = 0.01  # 1% error
                    tolerance_noise = None
                    for noise_level, error in error_pairs:
                        if error > tolerance_threshold:
                            tolerance_noise = noise_level
                            break
                    
                    if tolerance_noise:
                        max_noise_tolerance.append(tolerance_noise)
                    
                    print(f"    Lipschitz constant estimate: {lipschitz_estimate:.2e}")
                    print(f"    Noise tolerance (1% error): {tolerance_noise:.2e}" if tolerance_noise else "    Noise tolerance: Very high")
            
            # Overall assessment
            if lipschitz_constants:
                overall_lipschitz = np.max(lipschitz_constants)
                avg_tolerance = np.mean(max_noise_tolerance) if max_noise_tolerance else float('inf')
                
                # Good stability if Lipschitz constant is reasonable and tolerance is decent
                stability_good = overall_lipschitz < 100 and (not max_noise_tolerance or avg_tolerance > 1e-6)
                
                details = {
                    'max_lipschitz_constant': overall_lipschitz,
                    'average_noise_tolerance': avg_tolerance,
                    'sizes_tested': len(sizes),
                    'stability_assessment': 'Good' if stability_good else 'Poor'
                }
                
                exec_time = time.time() - start_time
                score = 1.0 if stability_good else 0.5
                self.log_result("Perturbation Bounds", stability_good, score, details, exec_time)
                
                return stability_good
            else:
                raise ValueError("No valid Lipschitz estimates computed")
    ...
    # =========================================================================
    # 3. CRYPTOGRAPHIC SOUNDNESS - Missing Tests  
    # =========================================================================
    
    def test_correlation_immunity(self) -> bool:
        """
        Test correlation immunity: ciphertext bits should not correlate 
        with plaintext bits under fixed key.
        """
        ...
        try:
            ...
            print(f"  Testing {num_plaintexts} plaintext/ciphertext pairs...")
            ...
            for i in range(num_plaintexts):
                ...
                # "Encrypt" using geometric hash (simplified)
                try:
                    result = engine.calculate_quantum_geometric_resonance(
                        list(plaintext), 
                        list(fixed_key)
                    )
                    
                    if 'geometric_hash' in result:
                        ciphertext = bytes.fromhex(result['geometric_hash'][:32])  # Take first 16 bytes
                    else:
                        # Fallback: use deterministic hash
                        hash_result = generate_deterministic_hash(list(plaintext), fixed_key)
                        ciphertext = bytes.fromhex(hash_result[:32])
            ...
            # Analyze bit-wise correlations
            max_correlation = 0.0
            correlation_count = 0
            
            for bit_pos in range(min(16, len(plaintexts[0])) * 8):  # Test up to 128 bits
                byte_pos = bit_pos // 8
                ...
                for pt, ct in zip(plaintexts, ciphertexts):
                    if byte_pos < len(pt) and byte_pos < len(ct):
                        pt_bit = (pt[byte_pos] >> bit_in_byte) & 1
                        ct_bit = (ct[byte_pos] >> bit_in_byte) & 1
                        
                        plaintext_bits.append(pt_bit)
                        ciphertext_bits.append(ct_bit)
                
                if len(plaintext_bits) > 10:  # Need sufficient data
                    # Calculate correlation
                    try:
                        correlation = np.corrcoef(plaintext_bits, ciphertext_bits)[0, 1]
            ...
            if correlations:
                ...
                # Good correlation immunity: low correlations
                immunity_good = max_correlation < 0.2 and avg_correlation < 0.1
                
                details = {
                    'max_bit_correlation': max_correlation,
                    'average_bit_correlation': avg_correlation,
                    'bits_tested': correlation_count,
                    'plaintexts_tested': len(plaintexts),
                    'immunity_quality': 'Good' if immunity_good else 'Poor'
                }
                
                print(f"    Max bit correlation: {max_correlation:.4f}")
                print(f"    Average bit correlation: {avg_correlation:.4f}")
                print(f"    Assessment: {'✅ Good immunity' if immunity_good else '❌ Poor immunity'}")
                
                exec_time = time.time() - start_time
                score = 1.0 if immunity_good else 0.3
    ...
    def test_entropy_retention(self) -> bool:
        """
        Test entropy retention in RFT+Cipher pipeline.
        Verify that the transform preserves or improves entropy.
        """
        ...
        try:
            # Test different input entropy levels
            input_entropies = []
            output_entropies = []
            entropy_ratios = []
            
            test_cases = [
                ("high_entropy", lambda n: secrets.token_bytes(n)),
                ("medium_entropy", lambda n: bytes([i % 256 for i in range(n)])),
                ("low_entropy", lambda n: b"A" * n),
                ("pattern", lambda n: bytes([int(127 * (1 + np.sin(2*np.pi*i/16))) for i in range(n)]))
            ]
            
            for case_name, data_gen in test_cases:
                print(f"  Testing {case_name}...")
                
                # Generate test data
                input_data = data_gen(32)  # 32 bytes = 256 bits
                
                # Measure input entropy (Shannon entropy of bytes)
                input_counts = np.bincount(list(input_data), minlength=256)
                input_probs = input_counts / len(input_data)
                input_entropy = -np.sum(input_probs[input_probs > 0] * np.log2(input_probs[input_probs > 0]))
                
                # Apply RFT transform
                try:
                    if RFT_AVAILABLE:
                        rft_result = forward_true_rft(list(input_data))
                        # Convert complex result back to bytes for entropy analysis
                        rft_bytes = []
                        for val in rft_result:
                            if isinstance(val, complex):
                                # Convert complex to bytes via real/imag parts
                                real_byte = int(abs(val.real) * 127) % 256
                                imag_byte = int(abs(val.imag) * 127) % 256
                                rft_bytes.extend([real_byte, imag_byte])
                            else:
                                # Convert real value to byte
                                byte_val = int(abs(val) * 127) % 256
                                rft_bytes.append(byte_val)
                        
                        output_data = bytes(rft_bytes[:32])  # Truncate to same length
                    else:
                        # Fallback: use deterministic hash
                        hash_result = generate_deterministic_hash(list(input_data))
                        output_data = bytes.fromhex(hash_result)[:32]
                    
                    # Measure output entropy
                    output_counts = np.bincount(list(output_data), minlength=256)
                    output_probs = output_counts / len(output_data)
                    output_entropy = -np.sum(output_probs[output_probs > 0] * np.log2(output_probs[output_probs > 0]))
                    
                    # Record measurements
                    input_entropies.append(input_entropy)
                    output_entropies.append(output_entropy)
                    
                    if input_entropy > 0:
                        entropy_ratio = output_entropy / input_entropy
                        entropy_ratios.append(entropy_ratio)
                        
                        print(f"    {case_name}: {input_entropy:.2f} → {output_entropy:.2f} bits (ratio: {entropy_ratio:.2f})")
                    else:
                        print(f"    {case_name}: {input_entropy:.2f} → {output_entropy:.2f} bits (ratio: N/A)")
                
                except Exception as inner_e:
                    print(f"    {case_name}: Failed - {inner_e}")
                    continue
            
            # Assessment
            if entropy_ratios:
                avg_ratio = np.mean(entropy_ratios)
                min_ratio = np.min(entropy_ratios)
                
                # Good entropy retention: ratios near 1.0 or higher  
                retention_good = avg_ratio > 0.8 and min_ratio > 0.5
                
                details = {
                    'average_entropy_ratio': avg_ratio,
                    'minimum_entropy_ratio': min_ratio,
                    'test_cases': len(entropy_ratios),
                    'retention_quality': 'Good' if retention_good else 'Poor'
                }
                ...
                raise ValueError("No valid entropy measurements")
    ...
    # =========================================================================
    # 4. COLLISION & PREIMAGE ANALYSIS - Missing Tests
    # =========================================================================
    
    def test_gradient_collision_search(self) -> bool:
        """
        Test resistance to gradient-based collision attacks.
        Try to find distinct inputs that produce same hash using optimization.
        """
        print("\n🎯 Testing Gradient-Based Collision Search")  
        print("-" * 45)
        ...
        try:
            if not SCIPY_AVAILABLE:
                ...
                return self.test_simple_collision_search()
            
            # Target: find x1 ≠ x2 such that H(x1) = H(x2)
            search_attempts = 5
            successful_collisions = 0
            search_times = []
            
            for attempt in range(search_attempts):
                print(f"  Attempt {attempt + 1}/{search_attempts}...")
                
                attempt_start = time.time()
                
                # Start with random initial point
                np.random.seed(42 + attempt)
                x1_initial = np.random.randn(16)  # 16D input space
                
                # Get target hash from x1
                try:
                    target_hash = generate_deterministic_hash(x1_initial.tolist())
                    target_bytes = bytes.fromhex(target_hash)
                except Exception as e:
                    print(f"    Failed to generate target hash: {e}")
                    continue
                
                # Define optimization objective: find x2 that minimizes |H(x2) - H(x1)|
                def collision_objective(x2):
                    try:
                        x2_hash = generate_deterministic_hash(x2.tolist())
                        x2_bytes = bytes.fromhex(x2_hash)
                        
                        # Hamming distance as objective
                        distance = sum(b1 != b2 for b1, b2 in zip(target_bytes, x2_bytes))
                        return distance
                    except:
                        return float('inf')
                ...
                def difference_constraint(x2):
                    return np.linalg.norm(x2 - x1_initial) - 1e-6  # Must differ by at least 1e-6
                
                # Run optimization
                try:
                    result = scipy.optimize.minimize(
                        collision_objective,
                        x1_initial + np.random.randn(16) * 0.1,  # Start near but different from x1
                        method='BFGS',
                        constraints={'type': 'ineq', 'fun': difference_constraint},
                        options={'maxiter': 100}  # Limit iterations to prevent long search
                    )
                    
                    if result.success and result.fun < 1:  # Found collision (Hamming distance < 1)
                        x2_final = result.x
                        
                        # Verify it's actually different and produces collision
...
if __name__ == "__main__":
    ...
    if all(results.values()):
        ...
        print("\n❌ Some advanced validation tests FAILED")
        sys.exit(1)
