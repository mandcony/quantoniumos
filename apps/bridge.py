...
ASSETS_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "attached_assets"
)
...
def output_reader(
    process: subprocess.Popen, output_queue: queue.Queue, process_id: str
):
    """
...
def launch_component(
    ...
    try:
        ...
        python_path = [ASSETS_DIR, os.path.dirname(os.path.abspath(__file__))]
        env["PYTHONPATH"] = (
            os.pathsep.join(python_path) + os.pathsep + env.get("PYTHONPATH", "")
        )
        ...
        if capture_output:
            ...
            threading.Thread(
                target=output_reader, args=(process, output_q, process_id), daemon=True
            ).start()
        else:
            process = subprocess.Popen(
                cmd, env=env, cwd=os.path.dirname(component_path)
            )
        ...
        for pid, process in list(active_processes.items()):
            try:
                ...
                try:
                    ...
                    logger.error(
                        f"Failed to kill process {pid} during cleanup: {str(e)}"
                    )
