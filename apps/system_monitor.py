...
class SystemMonitor:
    ...
    def create_overview_tab(self):
        ...
        self.cpu_threads_label = QLabel(f"CPU Threads: {psutil.cpu_count(logical=True)}")
    ...
    def update_cpu_info(self):
        ...
        try:
            ...
            if cpu_freq:
                self.cpu_freq_label.setText(f"CPU Frequency: {cpu_freq.current:.0f} MHz")
...
"""
    ...
    def update_memory_info(self):
        ...
        try:
            ...
            self.memory_available_label.setText(f"Available: {memory.available / (1024**3):.1f} GB")
...
"""
    ...
    def update_disk_info(self):
        ...
        try:
            ...
            self.disk_usage_label.setText(f"Disk Usage: {(disk.used / disk.total * 100):.1f}%")
            self.disk_free_label.setText(f"Free Space: {disk.free / (1024**3):.1f} GB")
            self.disk_total_label.setText(f"Total Space: {disk.total / (1024**3):.1f} GB")
    ...
    def update_network_info(self):
        ...
        try:
            ...
            self.network_sent_label.setText(f"Bytes Sent: {net_io.bytes_sent / (1024**2):.1f} MB")
            self.network_recv_label.setText(f"Bytes Received: {net_io.bytes_recv / (1024**2):.1f} MB")
    ...
    def refresh_processes(self):
        ...
        try:
            ...
            for row, proc in enumerate(processes[:50]):
                ...
                self.process_table.setItem(row, 2, QTableWidgetItem(f"{proc['cpu_percent'] or 0:.1f}"))
                self.process_table.setItem(row, 3, QTableWidgetItem(f"{proc['memory_percent'] or 0:.1f}"))
