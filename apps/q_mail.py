...
class QuantumMail(QWidget if PYQT5_AVAILABLE else object):
    ...
    def create_folder_panel(self, parent_splitter):
        ...
        self.folder_stats.setStyleSheet("color: #4169e1; font-weight: bold; margin: 5px;")
    ...
    def create_email_list_panel(self, parent_splitter):
        ...
        self.email_table.setHorizontalHeaderLabels(["From/To", "Subject", "Date", "Size"])
    ...
    def create_status_bar(self, parent_layout):
        ...
        self.status_label.setStyleSheet("color: #4169e1; font-weight: bold; margin: 5px;")
        ...
        self.encryption_indicator.setStyleSheet("color: #00ff88; font-weight: bold; margin: 5px;")
...
🔐 Your privacy is our priority""",
    ...
    def auto_refresh_mail(self):
        ...
        if self.mail_config.get('configured', False) and self.mail_config.get('auto_refresh', True):
            self.refresh_mail()
    ...
    def refresh_email_list(self):
        ...
        for row, email_data in enumerate(emails):
            ...
            self.email_table.setItem(row, 1, QTableWidgetItem(email_data.get('subject', 'No Subject')))
            self.email_table.setItem(row, 2, QTableWidgetItem(email_data.get('date', 'Unknown')))
            self.email_table.setItem(row, 3, QTableWidgetItem(email_data.get('size', '0 KB')))
        ...
        unread_count = sum(1 for folder in self.mail_data.values() 
                          for email in folder if not email.get('read', True))
        
        self.mail_stats.setText(f"📊 Messages: {total_messages} | Unread: {unread_count}")
    ...
    def delete_mail(self):
        ...
        if self.selected_message:
            reply = QMessageBox.question(self, "🗑️ Delete Email", 
            ...
            if reply == QMessageBox.Yes:
                ...
                emails = self.mail_data.get(self.current_folder, [])
                emails = [e for e in emails if e.get('id') != self.selected_message.get('id')]
    ...
    def toggle_encryption(self):
        ...
        if self.quantum_encryption:
            ...
            self.encryption_indicator.setStyleSheet("color: #ff6b35; font-weight: bold; margin: 5px;")
