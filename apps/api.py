...
from apps.bridge import (BridgeError, get_component_status, get_process_output,
                         is_process_running, launch_quantonium_os,
                         launch_quantum_nova, launch_resonance_analyzer,
                         launch_vibrational_engine, terminate_component)
...
def api_launch_nova():
    ...
    try:
        ...
        if qubit_count < 1 or qubit_count > 150:
            return (
                jsonify(
                    {"success": False, "error": "Qubit count must be between 1 and 150"}
                ),
                400,
            )
        ...
        return (
            jsonify({"success": False, "error": f"Invalid parameters: {str(e)}"}),
            400,
        )
...
def api_status():
    ...
    if process_id:
        ...
        return jsonify(
            {"process_id": process_id, "running": is_running, "info": api_info}
        )
    else:
        ...
        return jsonify({"processes": status, "count": len(status)})
