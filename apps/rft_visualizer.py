...
class RFTVisualizer(QWidget if PYQT5_AVAILABLE else object):
    ...
    def __init__(self):
        if not PYQT5_AVAILABLE:
            print("⚠️ PyQt5 required for RFT Visualizer GUI")
            return
    ...
    def init_ui(self):
        ...
        title.setStyleSheet("color: #1a1a1a; margin: 12px; background: rgba(255, 255, 255, 0.8); padding: 12px; border-radius: 12px; font-weight: 600; border: 1px solid rgba(255, 255, 255, 0.3);")
        ...
        # Status
        self.status_label = QLabel("✅ RFT Visualizer Ready")
        self.status_label.setStyleSheet("color: #dc2626; font-weight: 600; margin: 6px;")
    ...
    def create_control_panel(self, parent_layout):
        ...
        self.wave_type.addItems(["Sine", "Cosine", "Square", "Sawtooth", "Triangle", "White Noise"])
        ...
        # Progress indicator
        transform_layout.addWidget(QLabel("📊 Progress:"))
        ...
        self.avalanche_check = QCheckBox("❄️ Avalanche Effect")
        self.avalanche_check.setChecked(True)
        analysis_layout.addWidget(self.avalanche_check)
        
        self.frequency_check = QCheckBox("📊 Frequency Analysis")
    ...
    def create_visualization_panel(self, parent_layout):
        ...
        viz_group = QGroupBox("📊 RFT Visualizations")
    ...
    def create_status_panel(self, parent_layout):
        ...
        # Transformation time
        self.time_label = QLabel("⏱️ Time: 0.00s")
        self.time_label.setStyleSheet("color: #1a1a1a; font-weight: 600;")
        status_layout.addWidget(self.time_label)
        
        # Security level
        self.security_label = QLabel("🛡️ Security: High")
    ...
    def initialize_rft_system(self):
        ...
        self.status_label.setText("🌊 RFT System initialized - Ready for wave processing")
    ...
    def generate_wave_data(self):
        ...
        if wave_type == "Sine":
            ...
            self.input_data = np.sign(np.sin(2 * np.pi * base_freq * t)) * self.wave_amplitude
        elif wave_type == "Sawtooth":
            self.input_data = 2 * (t * base_freq - np.floor(t * base_freq + 0.5)) * self.wave_amplitude
        elif wave_type == "Triangle":
            self.input_data = 2 * np.arcsin(np.sin(2 * np.pi * base_freq * t)) / np.pi * self.wave_amplitude
        ...
        self.status_label.setText(f"🌊 Generated {wave_type.lower()} wave data ({len(self.input_data)} samples)")
    ...
    def update_frequency_resolution(self, new_resolution):
        ...
        self.status_label.setText(f"� Frequency resolution updated to {new_resolution}")
    ...
    def start_rft_transformation(self):
        ...
        stages = ['Windowing', 'FFT', 'Resonance Detection', 'Phase Analysis', 'Reconstruction']
        ...
        self.status_label.setText("✅ RFT transformation complete!")
        self.update_visualizations()
    ...
    def apply_rft_stage(self, data, stage_name):
        ...
        self.update_visualizations()
        self.status_label.setText("✅ RFT transformation completed")
    ...
    def step_rft_transformation(self):
        ...
        if current_stage >= 5:  # 5 RFT stages
            self.status_label.setText("✅ RFT processing already complete")
            return
        ...
        if current_stage < len(stages):
            ...
            self.update_visualizations()
            self.status_label.setText(f"⏭️ Completed stage: {stages[current_stage]}")
    ...
    def update_visualizations(self):
        ...
        if self.frequency_check.isChecked():
            ...
            ax4.set_title('📊 Byte Frequency Distribution', color='#ff6b35', fontweight='bold')
            ...
            if len(self.transformation_history) >= 2:
                for i, data in enumerate(self.transformation_history[-4:]):
                    ...
                    ax4.text(0.05, 0.8 - i*0.15, f'{stage_name}: {wave_info}', 
                            transform=ax4.transAxes, fontfamily='monospace',
                            color='#00ffcc', fontsize=9)
                ax4.set_title('🌊 Wave Analysis Data', color='#00ffcc', fontweight='bold')
    ...
    def update_metrics(self):
        ...
        resonance_count = len(self.resonance_peaks) if hasattr(self, 'resonance_peaks') else 0
        ...
        import time
        self.time_label.setText(f"⏱️ Time: {time.time() % 100:.2f}s")

def main():
    """Main entry point"""
    if not PYQT5_AVAILABLE:
        print("❌ PyQt5 required for RFT Visualizer")
        return
