...
def benchmark_vs_classical():
    ...
    for n in sizes:
        ...
        # RFT timing using actual implementation
        start_time = time.time()
        try:
            # Try to use actual RFT implementation
            from src.assembly.python_bindings.unitary_rft import UnitaryRFT
            rft = UnitaryRFT(n)
            rft_result = rft.forward(data)
            rft_time = time.time() - start_time
        except ImportError:
            # Fallback to simulated O(n) performance
