...
def inverse_resonance_fourier_transform(
    ...
    """
    if (
        "original_length" not in frequency_data
        or frequency_data["original_length"] == 0
    ):
    ...
    if N > 1:
        full_spectrum[n_positive:] = np.conj(
            complex_spectrum_positive[1 : N - n_positive + 1][::-1]
        )
    ...
    return {"waveform": reconstructed_signal.tolist(), "success": True}
...
def perform_irft_list(frequency_components: List[tuple]) -> List[float]:
    ...
    if original_signal_length is None:
        raise ValueError(
            "Frequency components list is missing the original signal length metadata."
        )
