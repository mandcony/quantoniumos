"""
Improved Geometric Waveform Hash with σ-tightening
...
def geometric_waveform_hash(msg: bytes, key: bytes, rounds: int = 3, rft=None) -> bytes:
    """
    Canonical geometric waveform hash with σ-tightening
    ...
    Returns:
        32-byte hash with target avalanche properties (μ~50%, σ≤2%)
    """
    if len(msg) == 0:
        msg = b'\x00'
