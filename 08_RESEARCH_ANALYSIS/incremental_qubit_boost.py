...
class IncrementalQubitAnalyzer:
    ...
    def analyze_incremental_scaling(
        self, start_qubits: int = 5, end_qubits: int = 25, step: int = 2
    ):
        """
    ...
    def generate_scaling_report(self) -> Dict[str, Any]:
        ...
        if "incremental" not in self.scaling_data:
            return {
                "error": "No scaling data available. Run analyze_incremental_scaling() first."
            }
...
def run_incremental_analysis():
    ...
    results = analyzer.analyze_incremental_scaling(
        start_qubits=5, end_qubits=25, step=2
    )
