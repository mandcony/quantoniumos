...
def setup_args():
    ...
    parser = argparse.ArgumentParser(description="Run E2E smoke tests for Quantonium OS API")
    parser.add_argument("--url", default=DEFAULT_BASE_URL, help="Base URL of the API")
    parser.add_argument("--api-key", help="API key for authentication")
    parser.add_argument("--skip-auth", action="store_true", help="Skip tests requiring authentication")
...
def test_metrics(base_url, headers):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"Metrics test failed with status code: {response.status_code}")
            return False
        
        data = response.json()
        logger.info(f"Metrics test successful! Process memory: {data.get('process', {}).get('memory_rss_bytes', 'N/A')} bytes")
        return True
    except Exception as e:
...
def test_encryption(base_url, headers):
    ...
    try:
        ...
        response = requests.post(f"{base_url}/api/encrypt", headers=headers, json=payload)
        if response.status_code != 200:
            logger.error(f"Encryption test failed with status code: {response.status_code}")
            logger.error(response.text)
            return None
...
def test_decryption(base_url, headers, ciphertext):
    ...
    try:
        ...
        response = requests.post(f"{base_url}/api/decrypt", headers=headers, json=payload)
        if response.status_code != 200:
            logger.error(f"Decryption test failed with status code: {response.status_code}")
            logger.error(response.text)
            return False
...
def test_openapi_spec(base_url):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"OpenAPI spec test failed with status code: {response.status_code}")
            return False
        ...
        logger.info(f"OpenAPI spec test successful! API: {title}, Version: {version}, Endpoints: {endpoints}")
        return True
    except Exception as e:
...
def test_docs_ui(base_url):
    ...
    try:
        ...
        if response.status_code != 200:
            logger.error(f"API docs UI test failed with status code: {response.status_code}")
            return False
