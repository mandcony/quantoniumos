    ...
    try:
        ...
        for table in tables:
            ...
            for idx_name, idx_def in indexes:
                ...
                new_idx_def = idx_def.replace(f"public.{table_name}", f"secure_api.{table_name}")
        ...
        with open('scripts/update_api_references.sql', 'w') as f:
            f.write("""-- SQL Migration Script to update API table references
...
""")
        ...
        with open('scripts/drop_original_api_tables.sql', 'w') as f:
            f.write("""-- IMPORTANT: Only execute this after validating the migration was successful
...
""")
        ...
        logger.info("Generated SQL migration scripts in scripts/update_api_references.sql")
        ...
        print("-----------------------")
        print("API key tables have been secured by moving them to a private schema with row-level security.")
        print("\nNext steps:")
        print("1. Update your application code to reference secure_api.api_keys instead of public.api_keys")
        print("2. Validate that the application works correctly with the new schema")
        print("3. Run scripts/drop_original_api_tables.sql to remove the original tables once verified")
        print("\nIf you encounter any issues, you can:")
        print("- Use the backup tables created in the public schema")
        print("- Run scripts/update_api_references.sql to create views for backward compatibility")
