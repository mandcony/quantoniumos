...
def validate_production_deployment():
    ...
    for cache_dir in cache_dirs:
        if Path(cache_dir).exists():
            warnings.append(f"Cache directory present: {cache_dir}")
    
    # Check 4: Validate secure configuration
    try:
        from core.security.secure_config import SecureConfig
        if not SecureConfig.validate_production_safety():
            errors.append("SecureConfig validation failed")
    except ImportError:
        errors.append("SecureConfig not available for validation")
...
if __name__ == "__main__":
    ...
    if not errors and not warnings:
        ...
        print("\\n🚫 Production deployment BLOCKED due to errors")
        sys.exit(1)
    else:
