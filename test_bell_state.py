...
def test_bell_state():
    ...
    print("||nAfter CNOT(0,1) - should be Bell state:")
    qc.apply_cnot(0, 1)
    probs = qc.get_state_probabilities()
    for state, prob in probs.items():
        if prob > 1e-6:
    ...
    # Check if it's the correct Bell state expected_00 = 0.5 expected_11 = 0.5 actual_00 = probs.get('00', 0) actual_11 = probs.get('11', 0)
