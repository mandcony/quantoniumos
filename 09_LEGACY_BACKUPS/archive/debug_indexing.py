...
"""
        ...
        # Start with |00>
        print("Initial state:") state = qc.get_computational_state() for i, amp in enumerate(state):
        if abs(amp) > 1e-10: binary = format(i, '02b')
        print(f" {amp:.6f}|||{binary}>")

        # Apply Hadamard to qubit 0
        print("||nAfter H(0):") qc.apply_hadamard(0) state = qc.get_computational_state() for i, amp in enumerate(state):
        if abs(amp) > 1e-10: binary = format(i, '02b')
        print(f" {amp:.6f}|||{binary}>")

        # Now manually apply CNOT matrix to check
        print(f"||nManual CNOT application:") cnot_result = CNOT @ state for i, amp in enumerate(cnot_result):
        if abs(amp) > 1e-10: binary = format(i, '02b')
        print(f" {amp:.6f}|||{binary}>")
...
if __name__ == "__main__": test_state_indexing() test_cnot_indexing() test_cnot_step_by_step()