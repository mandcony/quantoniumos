...
"""
    ...
    def test_enhanced_v2():
        print("=== Enhanced RFT Crypto v2 Performance Test ===") crypto = FixedRFTCryptoV2()

        # Test various sizes test_cases = [ b"A", # 1 byte b"AB", # 2 bytes b"Hello", # 5 bytes b"Hello, World!", # 13 bytes b"Hello, World! This is a test message!", # 37 bytes b"A" * 100, # 100 bytes b"B" * 1000, # 1000 bytes ] key = b"test_key_32_bytes_long_for_v2_test" for i, plaintext in enumerate(test_cases):
        print(f"\nTest {i+1}: {len(plaintext)} bytes")

        # Encrypt encrypted = crypto.encrypt(plaintext, key)
        print(f"Original: {len(plaintext)} bytes")
        print(f"Encrypted: {len(encrypted)} bytes")

        # Decrypt decrypted = crypto.decrypt(encrypted, key)
        print(f"Decrypted: {len(decrypted)} bytes")

        # Check match match = (decrypted == plaintext)
        print(f"Match: {'✓'
        if match else '✗'}")
        if not match:
...
if __name__ == "__main__": test_enhanced_v2()