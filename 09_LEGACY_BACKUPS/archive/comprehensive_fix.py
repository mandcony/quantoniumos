#!/usr/bin/env python3
"""
Comprehensive Test Module Fix === Recreate all test module headers with proper syntax.
"""

import os from pathlib
import Path

# Complete header with imports and utility functions COMPLETE_HEADER = '''
import numpy as np
import scipy.linalg
import math
...
"""
        ...
        # Find the class definition (this marks where the actual code starts) class_start = content.find('class ')
        if class_start == -1:
        print(f" ✗ Could not find class definition in {module_name}")
        return False

        # Extract class and method definitions (everything from class onwards) class_content = content[class_start:]

        # Combine header with class content new_content = COMPLETE_HEADER + '\n||n' + class_content

        # Write back with open(filepath, 'w') as f: f.write(new_content)
...
"""
        ...
        print("=" * 50)
        for filename in test_files: filepath = workspace_dir / filename
        if filepath.exists(): fix_module(str(filepath), filename)
        print("=" * 50)
