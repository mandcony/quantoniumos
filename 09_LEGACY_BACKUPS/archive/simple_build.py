...
def main():
        ...
        # Change to project root os.chdir('/workspaces/quantoniumos') success_count = 0

        # Build modules individually with specific sources modules = [ ("quantonium_core", '["core/pybind_interface.cpp", "core/engine_core.cpp", "core/symbolic_eigenvector.cpp"]'), ("resonance_engine", '["core/resonance_engine_bindings.cpp", "core/engine_core.cpp", "core/symbolic_eigenvector.cpp"]'), ("quantum_engine", '["core/quantum_engine_bindings.cpp", "core/engine_core.cpp", "core/symbolic_eigenvector.cpp"]') ] for name, sources in modules:
        try:
        if build_module(name, sources):
        print(f"✓ {name} built successfully") success_count += 1
        else:
        print(f"❌ {name} build failed") except Exception as e:
        print(f"❌ {name} build error: {e}")
        print(f"\n Built {success_count}/{len(modules)} modules")

        # Test what we built
        if success_count > 0:
        print("||n🧪 Testing built modules...") for name, _ in modules:
        try: cmd = [get_python_cmd(), "-c", f"import {name};
        print('✓ {name} imports successfully')"] result = subprocess.run(cmd, capture_output=True, text=True)
        if result.returncode == 0:
        print(result.stdout.strip())
        else:
        print(f"❌ {name}
import failed: {result.stderr}") except Exception as e:
