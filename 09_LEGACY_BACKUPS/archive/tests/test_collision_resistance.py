...
try:
    from core.encryption.geometric_waveform_hash import generate_waveform_hash, GeometricWaveformHash
    HASH_AVAILABLE = True
except ImportError:
...
class CollisionResistanceTester:
    ...
    def birthday_attack_test(self, num_samples: int = 10000) -> CollisionResult:
        ...
        for i in range(num_samples):
            ...
            if hash_output in hash_values:
                ...
                print(f"COLLISION FOUND: {collision_pair[0]} and {collision_pair[1]} -> {hash_output}")
            else:
        ...
        expected_collision_prob = 1 - math.exp(-num_samples**2 / (2 * 2**hash_bit_length))
        ...
        if len(collisions) == 0 and num_samples < expected_samples_for_collision / 10:
            ...
            security_assessment = "⚠ MARGINALLY SECURE (collisions within expected range)"
        else:
    ...
    def structured_collision_test(self) -> CollisionResult:
        ...
        if len(collisions) == 0:
            ...
            security_assessment = "✗ STRUCTURAL WEAKNESS (multiple structured collisions)"
    ...
    def prefix_collision_test(self) -> CollisionResult:
        ...
        if len(collisions) == 0:
            ...
            security_assessment = "⚠ PREFIX WEAKNESS (collisions found with structured prefixes)"
    ...
    def multicollision_test(self, target_collisions: int = 3) -> CollisionResult:
        ...
        if max_collision_size < target_collisions:
            security_assessment = f"✓ SECURE (no {target_collisions}-way collisions found)"
        elif max_collision_size == target_collisions:
            security_assessment = f"⚠ WEAKNESS ({target_collisions}-way collision found)"
        else:
            security_assessment = f"✗ MAJOR WEAKNESS ({max_collision_size}-way collision found)"
...
def run_comprehensive_collision_tests() -> str:
    ...
    for result in results:
        ...
        if result.collisions_found > 0:
            ...
            for i, (input1, input2) in enumerate(result.collision_pairs[:5]):  # Show first 5
                report += f" {i+1}. '{input1}' vs '{input2}'\n"
            if len(result.collision_pairs) > 5:
                report += f" ... and {len(result.collision_pairs) - 5} more\n"

        if "INSECURE" in result.security_assessment or "WEAKNESS" in result.security_assessment:
...
if __name__ == "__main__":
    ...
    result_clean = result.replace('✓', 'PASS').replace('❌', 'FAIL').replace('🔒', '').replace('⚡', '').replace('🎯', '').replace('✅', 'SUCCESS')
    print(result_clean)
