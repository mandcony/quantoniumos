...
def _hist_bin_doane(x, range):
    """"""
    Doane's histogram bin estimator. Improved version of Sturges' formula which works better for
    non-normal data. See
...
def _get_outer_edges(a, range):
    ...
    if range is not None:
        ...
        'stone'
            ...
            of Scott's rule. 'rice' Estimator does not take variability into account, only data size. Commonly overestimates number of bins required. 'sturges' R's default method, only accounts for data size. Only
    ...
    'sturges'
        ...
        larger, non-normal datasets. This is the default method in R's ``hist`` method. 'doane' .. math:: n_h = 1 + \log_{2}(n) + \log_{2}\left(1 + \frac{|g_1|}{\sigma_{g_1}}\right) g_1 = mean\left[\left(\frac{x - \mu}{\sigma}\right)^3\right] \sigma_{g_1} = \sqrt{\frac{6(n - 2)}{(n + 1)(n + 3)}} An improved version of Sturges' formula that produces better
...
def histogramdd(sample, bins=10, range=None, density=None, weights=None):
    ...
    sample : (N, D) array, or (N, D) array_like
        ...
        * When an array, each row is a coordinate in aD-dimensional space -
