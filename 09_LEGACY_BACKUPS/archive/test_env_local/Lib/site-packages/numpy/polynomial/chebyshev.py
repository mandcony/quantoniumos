...
def _zseries_to_cseries(zs):
    ...
    Convert az series to the equivalent Chebyshev series. The result is
    never an empty array. The dtype of the return is the same as that of
    the input. No checks are run on the arguments as this routine is for
    internal use.
...
def _zseries_mul(z1, z2):
    ...
    Multiply two z-series to produce az-series.
...
def chebroots(c):
    ...
    be improved by a few iterations of Newton's method. The Chebyshev series basis polynomials aren't powers of `x` so the
