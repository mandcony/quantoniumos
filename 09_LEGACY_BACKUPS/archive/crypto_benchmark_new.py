...
"""
        ...
        print("Testing RFT Symbolic Resonance Crypto...") rft_crypto = BenchmarkUtils.create_rft_crypto() rft_start = time.time() rft_differential_leakage = 0 for p1, p2 in plaintext_pairs:
        ...
        # Low differential = vulnerability rft_differential_leakage += 1 rft_time = time.time() - rft_start rft_vulnerability = rft_differential_leakage / num_tests results = { 'num_tests': num_tests, 'aes_vulnerability_rate': aes_vulnerability, 'rft_vulnerability_rate': rft_vulnerability, 'aes_time': aes_time, 'rft_time': rft_time, 'robustness_improvement': (aes_vulnerability - rft_vulnerability) / aes_vulnerability
        if aes_vulnerability > 0 else float('inf'), 'speed_ratio': aes_time / rft_time }

        # Print results BenchmarkUtils.print_results_table( ["Metric", "AES-256", "RFT", "Improvement"], [ ["Vulnerability Rate", f"{aes_vulnerability:.3f}", f"{rft_vulnerability:.3f}", f"{results['robustness_improvement']:.1f}×"], ["Time (seconds)", f"{aes_time:.3f}", f"{rft_time:.3f}", f"{results['speed_ratio']:.2f}×"] ] )
