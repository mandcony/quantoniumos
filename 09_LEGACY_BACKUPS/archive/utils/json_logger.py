...
def setup_json_logger(app, log_dir="logs", log_level=logging.INFO):
    ...
    # Create log directory if it doesn't exist if not os.path.exists(log_dir): os.makedirs(log_dir) # Configure the JSON formatter json_formatter = JSONFormatter() # Set up timed rotating file handler (rotate daily, keep 14 days) log_file_path = os.path.join(log_dir, 'quantonium_api.log') file_handler = TimedRotatingFileHandler( log_file_path, when='D', # Daily rotation interval=1, backupCount=14 ) # Create the log file to ensure it exists with open(log_file_path, 'a') as f: pass # File is created if it doesn't exist
