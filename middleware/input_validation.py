...
PATTERNS = {
    ...
    'hostname': re.compile(r'^(([a-zA-Z0-9]|[a-zA-Z0-9][a-zA-Z0-9\-]*[a-zA-Z0-9])\.)*([A-Za-z0-9]|[A-Za-z0-9][A-Za-z0-9\-]*[A-Za-z0-9])$'),
...
INJECTION_CHARS = [
    ...
    'truncate', 'insert', 'update', 'delete', 'exec', 'script',
    '<script>', '</script>', 'onerror', 'onload', 'eval(', 'javascript:',
            ...
            for field, field_schema in schema.items():
                ...
                if expected_type == 'string' and isinstance(value, str):
                    ...
                    if injection_patterns:
                        ...
                        if found_patterns:
                            errors.append(f"Field {field} contains suspicious patterns: {', '.join(found_patterns)}")
                        ...
                        for i, item in enumerate(value):
                            ...
                            if item_type:
                                ...
                                if item_type == 'string' and isinstance(item, str):
                                    valid_type = True
                                elif item_type == 'number' and isinstance(item, (int, float)):
                                    valid_type = True
                                ...
                                if not valid_type:
                                    errors.append(f"Item {i} in field {field} must be of type {item_type}")
        ...
        def wrapper(*args, **kwargs):
            ...
            for param_name, rules in param_rules.items():
                ...
                if check_injection and isinstance(value, str):
                    found_patterns = check_injection_attempts(value)
                    if found_patterns:
                        errors.append(f"Parameter {param_name} contains suspicious patterns: {', '.join(found_patterns)}")
        ...
        def wrapper(*args, **kwargs):
            ...
            for field_name, rules in field_rules.items():
                ...
                if check_injection:
                    found_patterns = check_injection_attempts(value)
                    if found_patterns:
                        errors.append(f"Field {field_name} contains suspicious patterns: {', '.join(found_patterns)}")
