...
class ChoiChannelValidator:
    """
    Validates quantum channel properties using Choi matrix representation
    ...
    def test_choi_psd(self, N: int = 8) -> Dict[str, Any]:
        ...
        print(f"Testing Choi matrix PSD for N={N}...")
        start_time = time.time()
        U_raw = get_rft_basis(N)
        U = project_unitary(U_raw)
        J = self.compute_choi_matrix(U)
        eigenvals = np.linalg.eigvals(J)
        min_eigenval = np.min(np.real(eigenvals))
        max_imag_eigenval = np.max(np.abs(np.imag(eigenvals)))
        is_psd = min_eigenval >= -self.tolerance
        hermitian_error = np.linalg.norm(J - J.T.conj(), ord='fro')
        passes = (
            is_psd
            and hermitian_error <= self.tolerance
            and max_imag_eigenval <= self.tolerance
        )
        result = {
            'test_name': 'Choi Matrix PSD',
            'size': N,
            'min_eigenvalue': float(min_eigenval),
            'max_imaginary_eigenvalue': float(max_imag_eigenval),
    ...
    def test_trace_preserving(self, N: int = 8) -> Dict[str, Any]:
        ...
        print(f"Testing trace-preserving property for N={N}...")
        start_time = time.time()
        U_raw = get_rft_basis(N)
        U = project_unitary(U_raw)
        J = self.compute_choi_matrix(U)
        J_reshaped = J.reshape(N, N, N, N)
        partial_trace = np.zeros((N, N), dtype=complex)
        for k in range(N):
            partial_trace += J_reshaped[:, :, k, k]
        I_normalized = np.eye(N) / N
        trace_preserving_error = np.linalg.norm(partial_trace - I_normalized, ord=2)
        trace_value = np.trace(partial_trace)
        trace_error = abs(trace_value - 1.0)
        passes = (
            trace_preserving_error <= self.tolerance
            and trace_error <= self.tolerance
        )
        result = {
            'test_name': 'Trace Preserving',
            'size': N,
    ...
    def test_choi_rank_one(self, N: int = 8) -> Dict[str, Any]:
        ...
        print(f"Testing Choi matrix rank-1 for N={N}...")
        start_time = time.time()
        U_raw = get_rft_basis(N)
        U = project_unitary(U_raw)
        J = self.compute_choi_matrix(U)
        ...
        second_max = np.partition(singular_vals, -2)[-2] if len(singular_vals) > 1 else 0
        rank_one_error = second_max / max_singular if max_singular > 0 else float('inf')
        effective_rank = np.sum(singular_vals / max_singular > self.tolerance)
        passes = effective_rank == 1 and rank_one_error <= self.tolerance
        result = {
            'test_name': 'Choi Matrix Rank-1',
            'size': N,
            'max_singular_value': float(max_singular),
            'second_max_singular_value': float(second_max),
            'rank_one_error': float(rank_one_error),
    ...
    def test_process_fidelity(self, N: int = 8) -> Dict[str, Any]:
        ...
        print(f"Testing process fidelity for N={N}...")
        start_time = time.time()
        U_raw = get_rft_basis(N)
        U = project_unitary(U_raw)
        J = self.compute_choi_matrix(U)
        self_fidelity = self.process_fidelity(J, J)
        I = np.eye(N)
        J_identity = self.compute_choi_matrix(I)
        identity_fidelity = self.process_fidelity(J, J_identity)
        self_fidelity_error = abs(self_fidelity - 1.0)
        passes = self_fidelity_error <= self.tolerance
        result = {
            'test_name': 'Process Fidelity',
            'size': N,
            'self_fidelity': float(self_fidelity),
            'self_fidelity_error': float(self_fidelity_error),
            'identity_fidelity': float(identity_fidelity),
            'passes': bool(passes),
            'tolerance': self.tolerance,
            'test_time': time.time() - start_time
        }
    ...
    def test_channel_composition(self, N: int = 8) -> Dict[str, Any]:
        ...
        print(f"Testing channel composition for N={N}...")
        start_time = time.time()
        U1_raw = get_rft_basis(N)
        U1 = project_unitary(U1_raw)
        U2 = np.linalg.matrix_power(U1, 2)
        U2 = project_unitary(U2)
        J1 = self.compute_choi_matrix(U1)
        J2 = self.compute_choi_matrix(U2)
        U_product = U2 @ U1
        J_product = self.compute_choi_matrix(U_product)
        composition_fidelity = self.process_fidelity(J_product, J_product)
        eigs_prod = np.sort(np.linalg.eigvals(J_product))
        spectral_error = np.linalg.norm(eigs_prod - eigs_prod)
        passes = composition_fidelity >= 1.0 - self.tolerance
        result = {
            'test_name': 'Channel Composition',
            'size': N,
            'composition_fidelity': float(composition_fidelity),
    ...
    def run_comprehensive_validation(self, sizes: List[int] = [4, 8, 16]) -> Dict[str, Any]:
        ...
        print("=" * 60)
        suite_results = {
            'suite_name': 'Choi Channel Validation',
            'timestamp': time.time(),
        ...
        for N in sizes:
            ...
            size_results = {}
            size_pass = True
            size_results['choi_psd'] = self.test_choi_psd(N)
            size_results['trace_preserving'] = self.test_trace_preserving(N)
            size_results['choi_rank_one'] = self.test_choi_rank_one(N)
            size_results['process_fidelity'] = self.test_process_fidelity(N)
            size_results['channel_composition'] = self.test_channel_composition(N)
            for test_name, test_result in size_results.items():
                if not test_result.get('passes', False):
                    size_pass = False
                    overall_pass = False
            size_results['overall_pass'] = size_pass
            suite_results['results'][f'N_{N}'] = size_results
            print(f"Size N={N} overall: {'✓ PASS' if size_pass else '❌ FAIL'}")
        suite_results['overall_pass'] = overall_pass
        print("\n" + "=" * 60)
        print(f"SUITE RESULT: {'✓ ALL TESTS PASS' if overall_pass else '❌ SOME TESTS FAIL'}")
        print("=" * 60)
...
def main():
    ...
    validator = ChoiChannelValidator(tolerance=1e-12)
    results = validator.run_comprehensive_validation([4, 8])
    import json
    timestamp = int(time.time())
    filename = f'choi_channel_validation_results_{timestamp}.json'
