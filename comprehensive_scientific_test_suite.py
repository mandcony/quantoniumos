...
# Import actual C++ engines
cpp_engines_available = {}
try:
    import resonance_engine
    cpp_engines_available['resonance_engine'] = resonance_engine
    print("✅ C++ resonance_engine loaded successfully")
except ImportError as e:
    print(f"⚠️ C++ resonance_engine not available: {e}")

try:
    import quantonium_core
    cpp_engines_available['quantonium_core'] = quantonium_core
    print("✅ C++ quantonium_core loaded successfully")
except ImportError as e:
    print(f"⚠️ C++ quantonium_core not available: {e}")

try:
    import quantum_engine
    cpp_engines_available['quantum_engine'] = quantum_engine
    print("✅ C++ quantum_engine loaded successfully")
except ImportError as e:
    print(f"⚠️ C++ quantum_engine not available: {e}")

try:
    import engine_core_pybind
    cpp_engines_available['engine_core_pybind'] = engine_core_pybind
    print("✅ C++ engine_core_pybind loaded successfully")
except ImportError as e:
    print(f"⚠️ C++ engine_core_pybind not available: {e}")
...
if RFT_ENGINES_AVAILABLE:
    print(f"✅ C++ engines available: {list(cpp_engines_available.keys())}")
else:
...
try:
    ...
    print("✅ QuantoniumOS Core Engine loaded")
except ImportError:
    QUANTONIUM_CORE_AVAILABLE = False
    print("⚠️ QuantoniumOS Core not available")
...
class ScientificRFTTestSuite:
    ...
    def __init__(self, config: TestConfiguration):
        self.config = config
        self.results = {}
        self.mathematical_validator = MathematicalRFTValidator(config.precision_tolerance)
    ...
    def test_asymptotic_complexity_analysis(self) -> Dict[str, Any]:
        ...
        print("   Testing RFT computational scaling with Python orchestration + C++ acceleration...")
        ...
        for n in test_dimensions:
            ...
            roundtrip_error = np.linalg.norm(test_signal - reconstructed) / np.linalg.norm(test_signal)
        ...
        if len(forward_times) >= 3:
            ...
            for i in range(1, len(test_dimensions)):
                ...
                expected_ratio = n_ratio * np.log2(test_dimensions[i]) / np.log2(test_dimensions[i-1])
            ...
            print(f"   Average scaling ratio (actual/expected): {avg_scaling_ratio:.2f}")
            ...
            acceleration_modes = [status['acceleration_mode'] for status in results['acceleration_status']]
            cpp_count = sum(1 for mode in acceleration_modes if 'C++' in mode)
            print(f"   C++ acceleration used in {cpp_count}/{len(acceleration_modes)} tests")
        ...
        forward_poly = np.polyfit(np.log2(test_dimensions), np.log2(results['forward_times']), 1)
        inverse_poly = np.polyfit(np.log2(test_dimensions), np.log2(results['inverse_times']), 1)
        ...
        results['complexity_assessment_details'] = {
            ...
            'dsp_practical': results['forward_is_nlogn_compatible'] and results['inverse_is_nlogn_compatible'],
    ...
    def test_orthogonality_stress_test(self) -> Dict[str, Any]:
        ...
        """
        large_dimensions = [2**i for i in range(10, 12)]  # Up to 2048, limited for practical testing
        ...
        for n in large_dimensions:
            try:
                ...
                print(f"   N={n}: Orthogonality error={ortho_error:.2e}, Condition={cond_num:.2e}")
        ...
        if results['orthogonality_errors']:
            ...
            results['precision_stable'] = results['max_orthogonality_error'] < 1e-6
            results['largest_stable_dimension'] = max(results['dimensions_tested'])
            results['scalability_assessment'] = 'EXCELLENT' if results['precision_stable'] else 'LIMITED'
    ...
    def test_generalized_parseval_theorem(self) -> Dict[str, Any]:
        ...
        signal_tests = [
            ...
            ('Sinusoidal', np.sin(2 * np.pi * np.arange(64) / 64) + 1j * np.cos(2 * np.pi * np.arange(64) / 64)),
            ('Exponential Decay', np.exp(-np.arange(64) / 10) * (np.random.randn(64) + 1j * np.random.randn(64))),
        ...
        for signal_name, signal in signal_tests:
            ...
            results['parseval_violations'].append(energy_error > self.config.precision_tolerance)
            
            print(f"   {signal_name}: Energy error = {energy_error:.2e}")
        
        # Overall assessment
        results['max_energy_error'] = max(results['energy_conservation_errors'])
        results['parseval_theorem_holds'] = results['max_energy_error'] < self.config.precision_tolerance
    ...
    def test_compression_benchmarks(self) -> Dict[str, Any]:
        ...
        print("   Comparing RFT vs FFT compression efficiency with real implementations...")
        
        N = 128
        
        # Test signal types
        test_signals = [
            ('Smooth Polynomial', np.polynomial.polynomial.polyval(np.linspace(0, 1, N), [1, 2, -1, 0.5])),
        ...
        for signal_name, signal in test_signals:
            ...
            # Quality metrics
            rft_snr = 20 * np.log10(np.linalg.norm(signal) / (np.linalg.norm(signal - rft_reconstructed) + 1e-12))
            fft_snr = 20 * np.log10(np.linalg.norm(signal) / (np.linalg.norm(signal - fft_reconstructed) + 1e-12))
        ...
        results['rft_advantage'] = results['average_rft_snr'] - results['average_fft_snr']
        results['compression_effective'] = results['rft_advantage'] > 0
        
        # Performance analysis
        valid_cpp_times = [t for t in results['cpp_rft_times'] if t != float('inf')]
        valid_python_times = [t for t in results['python_rft_times'] if t != float('inf')]
        
        if valid_cpp_times and valid_python_times:
            results['cpp_vs_python_speedup'] = np.mean(valid_python_times) / np.mean(valid_cpp_times)
        else:
    ...
    def test_filter_design_analysis(self) -> Dict[str, Any]:
        ...
        for filter_type in results['filter_types']:
            ...
            # Performance metrics
            rft_response = np.abs(kernel.forward_rft(rft_filtered_signal.astype(complex)))
    ...
    def test_entropy_randomness_analysis(self) -> Dict[str, Any]:
        ...
        frequency_stat = abs(ones_count - len(bit_array)/2) / np.sqrt(len(bit_array)/4)
        ...
        expected_runs = 2 * ones_count * (len(bit_array) - ones_count) / len(bit_array) + 1
        runs_variance = 2 * ones_count * (len(bit_array) - ones_count) * (2 * ones_count * (len(bit_array) - ones_count) - len(bit_array)) / (len(bit_array)**2 * (len(bit_array) - 1))
        ...
        # Overall assessment
        tests_passed = sum([test['passed'] for test in results['test_results'].values()])
        ...
        print(f"   Cryptographic quality: {'ACCEPTABLE' if results['overall_randomness_quality']['cryptographically_secure'] else 'QUESTIONABLE'}")
    ...
    def test_cryptographic_primitives(self) -> Dict[str, Any]:
        ...
        for i, data in enumerate(test_data):
            ...
            success = reconstruction_error < 1e-6
        ...
        deterministic = np.allclose(hash_outputs[0], hash_outputs[2])
        
        # 2. Sensitivity: different inputs give different outputs
        sensitive = not np.allclose(hash_outputs[0], hash_outputs[1])
        case_sensitive = not np.allclose(hash_outputs[0], hash_outputs[3])
        ...
        results['encryption_tests']['functionally_correct'] = encryption_success_rate > 0.8
        
        print(f"   Encryption success rate: {encryption_success_rate:.1%}")
        print(f"   Hash function quality: {'GOOD' if results['hash_function_tests']['hash_quality'] else 'POOR'}")
    ...
    def test_large_scale_entanglement_simulation(self) -> Dict[str, Any]:
        ...
        results = {
            ...
            'qubit_counts_tested': [4, 8, 12],  # Limited to avoid memory issues (2^12 = 4096)
        ...
        for n_qubits in results['qubit_counts_tested']:
            ...
            if n_qubits % 2 == 0:
                ...
                entanglement_entropy = -np.sum(eigenvals * np.log2(eigenvals + 1e-12))
            else:
                entanglement_entropy = 0  # Simplified for odd qubit numbers
            ...
            print(f"   {n_qubits} qubits: Fidelity={fidelity:.3f}, Bell parameter={bell_parameter:.3f}")
        
        # Overall assessment
        results['large_scale_feasible'] = all(results['state_preparation_success'].values())
        results['average_fidelity'] = np.mean(list(results['entanglement_fidelities'].values()))
        results['quantum_advantage_demonstrated'] = any(bell > 2.0 for bell in results['bell_violations'].values())
    ...
    def test_decoherence_error_models(self) -> Dict[str, Any]:
        ...
        print("   Testing quantum state robustness under noise with real implementations...")
        ...
        for noise_level in results['noise_levels']:
            ...
            if cpp_quantum_engine:
                try:
                    quantum_engine_instance = cpp_quantum_engine.ResonanceFourierEngine()
                except Exception as e:
            ...
            for t in range(time_steps):
                ...
                if cpp_quantum_engine and N <= 2048:
                    try:
                        start = time.perf_counter()
                        rft_state = quantum_engine_instance.forward_true_rft(current_state)
                        cpp_time_total += time.perf_counter() - start
                        engine_used = 'cpp'
                        
                    except Exception as e:
                        if t == 0:  # Only print once
                            print(f"     C++ engines failed: {e}, using Python fallback")
                        start = time.perf_counter()
                ...
                if engine_used == 'cpp':
                    try:
                        start = time.perf_counter()
                        current_state = quantum_engine_instance.inverse_true_rft(noisy_rft_state)
                        cpp_time_total += time.perf_counter() - start
                    except:
                        start = time.perf_counter()
                ...
                current_state = current_state / np.linalg.norm(current_state)  # Renormalize
            ...
            print(f"     Coherence time: {coherence_time} steps (engine: {engine_used})")
        
        # Performance comparison
        if results['cpp_decoherence_times'] and results['python_decoherence_times']:
            results['decoherence_speedup'] = np.mean(results['python_decoherence_times']) / np.mean(results['cpp_decoherence_times'])
        else:
            results['decoherence_speedup'] = 0.0
        
        # Analysis
        results['robust_against_weak_noise'] = results['coherence_times'][0] > 10  # First noise level
        results['coherence_scaling'] = np.polyfit(results['noise_levels'], results['coherence_times'], 1)[0]
        results['decoherence_resistance'] = 'HIGH' if results['coherence_scaling'] < -20 else 'MODERATE'
        results['actual_engines_used'] = results['engine_usage'] in ['cpp', 'python_fallback']
    ...
    def test_channel_capacity_analysis(self) -> Dict[str, Any]:
        ...
        for snr_db in results['snr_levels_db']:
            ...
            noise = np.sqrt(noise_power/2) * (np.random.randn(len(rft_signal)) + 1j * np.random.randn(len(rft_signal)))
            ...
            # Measure mutual information (simplified)
            rft_errors = np.sum(information_bits != rft_decoded_bits)
            rft_error_rate = rft_errors / num_symbols
            
            # RFT capacity estimate
            if rft_error_rate < 0.5:
                rft_capacity = 1 + rft_error_rate * np.log2(rft_error_rate) + (1-rft_error_rate) * np.log2(1-rft_error_rate)
            else:
                rft_capacity = 0
            ...
            print(f"   SNR={snr_db}dB: RFT capacity={rft_capacity:.3f}, Classical={classical_capacity:.3f}")
        ...
        results['high_snr_performance'] = results['capacity_advantage'][-1] > 0  # Last (highest) SNR
    ...
    def test_information_geometry(self) -> Dict[str, Any]:
        ...
        try:
            ...
            rft_real_coords = np.column_stack([np.real(rft_coordinates), np.imag(rft_coordinates)])
            ...
            for i in range(min(50, num_states)):  # Sample subset for efficiency
                ...
                distances = [np.linalg.norm(point - rft_real_coords[j]) for j in range(num_states) if j != i]
                distances.sort()
                ...
                if len(distances) >= 3 and distances[2] > 0:
                    ...
                    estimated_dims.append(max(1, min(16, dim_estimate)))  # Clamp to reasonable range
        ...
        print(f"   Estimated intrinsic dimension: {results['geometric_properties']['estimated_intrinsic_dimension']:.1f}")
    ...
    def run_comprehensive_scientific_validation(self) -> Dict[str, Any]:
        ...
        for domain, tests in comprehensive_results['domain_results'].items():
            ...
            for test_name, test_result in tests.items():
                ...
                # Simple success criteria (can be refined per test)
                if 'all_tests_passed' in test_result and test_result['all_tests_passed']:
                    domain_success += 1
                    successful_tests += 1
                elif any(key.endswith('_successful') or key.endswith('_passed') or key.endswith('_effective') 
                        for key in test_result.keys() 
                        if isinstance(test_result.get(key), bool) and test_result[key]):
                    domain_success += 1
                    successful_tests += 1
        ...
        print(f"Scientific Validity: {'CONFIRMED' if comprehensive_results['scientific_assessment']['scientific_validity'] else 'QUESTIONABLE'}")
        print(f"Industrial Readiness: {'YES' if comprehensive_results['scientific_assessment']['industrial_readiness'] else 'NO'}")
...
if __name__ == "__main__":
    ...
    print(f"Results available in comprehensive test output.")
