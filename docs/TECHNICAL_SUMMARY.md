...
| --- | --- | --- | --- |
| GPT‑OSS quantum sample | `ai/models/quantum/quantonium_120b_quantum_states.json` | `python - <<'PY'` ... `json.load(...)` → 4 096 symbolic states. File contains statistical metadata only. | 2.3 MB symbolic JSON - NOT reconstructible weights, no inference capability validated. Parameter counts in metadata are theoretical. Generated by `tools/generate_gpt_oss_quantum_sample.py`. |
