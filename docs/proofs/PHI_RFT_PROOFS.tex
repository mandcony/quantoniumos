...
\subsection{Canonical Gram-Normalized RFT}

The \emph{canonical} RFT is constructed via Gram-matrix normalization (symmetric orthogonalization) of an irrational-frequency exponential basis.

\begin{definition}[Irrational Frequency Basis]\label{def:kernel}
The raw basis matrix $\Phi \in \C^{n \times n}$ is defined by:
\begin{equation}
...
The canonical RFT matrix $\widetilde{\Phi} \in \C^{n \times n}$ is obtained via Gram-matrix normalization:
\begin{equation}
\widetilde{\Phi} = \Phi (\Phi\dg \Phi)^{-1/2}
\end{equation}
where $(\Phi\dg \Phi)^{-1/2}$ is the inverse square root of the Gram matrix $G = \Phi\dg \Phi$.
\end{definition}

\begin{theorem}[Canonical RFT Unitarity]\label{thm:canonical_unitary}
The canonical RFT matrix $\widetilde{\Phi}$ is exactly unitary.
\end{theorem}

\begin{proof}
Let $G = \Phi\dg \Phi$ be the Gram matrix. Since $\Phi$ is full-rank (generically true), $G$ is Hermitian positive definite.
\begin{equation}
\widetilde{\Phi}\dg \widetilde{\Phi} = (G^{-1/2})\dg \Phi\dg \Phi G^{-1/2} = G^{-1/2} G G^{-1/2} = I_n
\end{equation}
This construction (Loewdin orthogonalization) yields the unique unitary matrix closest to $\Phi$ in the Frobenius norm.
\end{proof}
