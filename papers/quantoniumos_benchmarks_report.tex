...
\begin{itemize}
    \item \textbf{Scalability}: QuantoniumOS achieves 10 million qubit compression where classical simulators require $2^{10,000,000}$ amplitudes (physically impossible)
    \item \textbf{Throughput}: Sustained 19-23 Mq/s (million qubits per second)
...
\begin{itemize}
    ...
    \item \textbf{Security basis}: Short Integer Solution (SIS) problem
    \item \textbf{Estimated security}: $\sim$128-bit equivalent (\textit{no formal security proofs, no peer-reviewed cryptanalysis})
\end{itemize}

\textbf{Security Disclaimer}: RFT-SIS parameters are inspired by NIST Kyber but have NOT undergone formal security reduction proofs or professional cryptanalysis. Security level estimates are extrapolations based on lattice dimensions. \textit{DO NOT use in production without expert cryptographic review.}
...
\textbf{RFT-SIS+Feistel} & \textbf{$\sim$256-bit} & \textbf{$\sim$128-bit**} & \textbf{Research/Unproven} \\
\bottomrule
\multicolumn{4}{l}{\small * Grover's algorithm halves symmetric key size} \\
\multicolumn{4}{l}{\small ** Estimated only, no security proofs, no cryptanalysis performed}
\end{tabular}
\end{table}
...
\midrule
A (Quantum) & \textbf{10M qubits} & O(n) vs O($2^n$), 19.1 Mq/s sustained \\
...
\begin{enumerate}
    \item \textbf{Quantum scalability}: O(n) symbolic compression reaching 10 million qubits
