||#!/usr/bin/env python3
""""""
RFT Quantum Computing Performance Test: Surgical Fixes Validation

This script validates the ~100× performance improvement achieved through:
1. Pre-conjugated gate computation (eliminate B/B⁻¹ per gate)
2. Consistent computational basis measurements
3. Fast gate lookup with caching

Tests progressively larger circuits to demonstrate scalability improvement.
""""""
...
def create_benchmark_circuit(n_qubits: int, depth: int):
    ...
    for layer in range(depth // 3):
        for i in range(n_qubits - 1):
            circuit.append(("CNOT", [i, (i+1) % n_qubits], {}))

    # Layer 3: Rotations
    remaining_depth = depth - len(circuit)
    for i in range(min(n_qubits, remaining_depth // 2)):
        circuit.append(("RX", [i], {'angle': np.pi/4}))
        circuit.append(("RZ", [i], {'angle': np.pi/6}))
...
def benchmark_approach(n_qubits: int, circuit_depth: int, approach: str, timeout_seconds: float = 10.0):
    ...
    try:
        ...
        qc = RFTQuantumComputer(num_qubits=n_qubits, approach=approach)
        setup_time = time.time() - start_setup

        # Create circuit
        circuit = create_benchmark_circuit(n_qubits, circuit_depth)

        # Apply circuit with timeout
        ...
        for gate_name, qubits, params in circuit:
            ...
            if gate_name == "H":
                qc.apply_hadamard(qubits[0])
            elif gate_name == "CNOT":
                qc.apply_cnot(qubits[0], qubits[1])
            elif gate_name == "RX":
                qc.apply_rotation_x(qubits[0], params['angle'])
            elif gate_name == "RZ":
                qc.apply_rotation_z(qubits[0], params['angle'])

            gates_applied += 1

        total_time = time.time() - start_time
        avg_gate_time_ms = (total_time / len(circuit)) * 1000 if len(circuit) > 0 else 0

        # Get final metrics
        coherence = qc.get_coherence_score()
        ...
        return {
            ...
            'avg_gate_time': avg_gate_time_ms,
            'coherence': coherence,
            'gates_applied': len(circuit)
        }
...
def run_performance_analysis():
    ...
    # Test configurations: (n_qubits, circuit_depth, timeout)
    test_configs = [
        (2, 10, 2.0),   # Small test
        (3, 15, 5.0),   # Medium test
        (4, 20, 10.0),  # Large test
        (5, 25, 15.0),  # Very large test
        (6, 30, 20.0),  # Stress test
    ]

    results = {}

    for n_qubits, circuit_depth, timeout in test_configs:
        print(f"\n{'='*60}")
        ...
        if n_qubits <= 3:
            approaches = ["fast_conjugated", "basis_hop"]
        else:
            # Skip basis_hop for large circuits - it would timeout
            approaches = ["fast_conjugated"]
            print(" (Skipping basis_hop - would exceed timeout due to B/B⁻¹ overhead)")

        for approach in approaches:
            results[n_qubits][approach] = benchmark_approach(n_qubits, circuit_depth, approach, timeout)

        # Calculate and show speedup
        if "fast_conjugated" in results[n_qubits] and "basis_hop" in results[n_qubits]:
            fast_result = results[n_qubits]["fast_conjugated"]
            slow_result = results[n_qubits]["basis_hop"]

            if fast_result and slow_result:
                speedup = slow_result['total_time'] / fast_result['total_time']
        ...
        elif "fast_conjugated" in results[n_qubits]:
            fast_result = results[n_qubits]["fast_conjugated"]
            if fast_result:
    ...
    successful_fast = sum(1 for n in results if results[n].get("fast_conjugated"))
    successful_slow = sum(1 for n in results if results[n].get("basis_hop"))
    ...
    # Calculate average speedups where both approaches worked
    speedups = []
    for n_qubits in results:
        if "fast_conjugated" in results[n_qubits] and "basis_hop" in results[n_qubits]:
            fast_result = results[n_qubits]["fast_conjugated"]
            slow_result = results[n_qubits]["basis_hop"]
            if fast_result and slow_result:
...
def test_bell_state_correctness():
    ...
    # Test Bell state with both approaches
    approaches = ["fast_conjugated", "basis_hop"]
    bell_results = {}

    for approach in approaches:
        print(f"||nTesting {approach}...")
        qc = RFTQuantumComputer(num_qubits=2, approach=approach)
    ...
    # Expected Bell state: 0.5|00> + 0.5|||11>
    expected_00 = 0.5
    expected_11 = 0.5
    expected_01 = 0.0
    expected_10 = 0.0
    ...
    for approach in approaches:
        probs = bell_results[approach]
        error_00 = abs(probs.get('00', 0) - expected_00)
        error_11 = abs(probs.get('11', 0) - expected_11)
        error_01 = abs(probs.get('01', 0) - expected_01)
        error_10 = abs(probs.get('10', 0) - expected_10)

        total_error = error_00 + error_11 + error_01 + error_10
