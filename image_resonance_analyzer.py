...
class ImageResonanceAnalyzer:
    ...
    def analyze_geometric_patterns(self):
        ...
        circle_candidates = (circle_response == maxima) & (circle_response > np.percentile(circle_response, 90))
    ...
    def _calculate_symmetry(self):
        ...
        h_sym = np.mean(1 - np.abs(self.grayscale.astype(float) - h_flip.astype(float)) / 255)
        
        # Check vertical symmetry
        v_flip = np.flipud(self.grayscale)
        v_sym = np.mean(1 - np.abs(self.grayscale.astype(float) - v_flip.astype(float)) / 255)
        
        # Check rotational symmetry (90 degrees)
        rot_90 = np.rot90(self.grayscale)
        r_sym = np.mean(1 - np.abs(self.grayscale.astype(float) - rot_90.astype(float)) / 255)
    ...
    def extract_waveforms(self):
        ...
        for angle in range(0, 360, 45):
            ...
            for r in range(0, min(center_y, center_x), 2):
                ...
                if 0 <= y < self.grayscale.shape[0] and 0 <= x < self.grayscale.shape[1]:
    ...
    def analyze_resonance_patterns(self):
        ...
        for idx, waveform in enumerate(self.edge_waveforms):
            ...
            strength = rft_result["amplitudes"][resonant_freqs] / np.max(rft_result["amplitudes"])
    ...
    def _check_phase_pattern(self, phase_diffs):
        ...
        harmony_count = sum(1 for diff in phase_diffs if min(diff % (np.pi/4), (np.pi/4) - diff % (np.pi/4)) < 0.1)
        
        # Check for golden ratio alignment
        golden = (1 + np.sqrt(5)) / 2
        golden_count = sum(1 for diff in phase_diffs if min(abs(diff - np.pi/golden), abs(diff - 2*np.pi/golden)) < 0.2)
    ...
    def visualize_analysis(self):
        ...
        if self.edge_waveforms:
            ...
            axs[1, 1].scatter(resonant_indices % len(sample_waveform), resonant_values, color='red')
        ...
        if hasattr(self, 'phase_relationships') and self.phase_relationships:
            ...
            strengths = [data["avg_strength"] for data in self.phase_relationships.values()]
            patterns = [data["phase_pattern"] for data in self.phase_relationships.values()]
            
            colors = {'harmonic': 'green', 'golden_ratio': 'gold', 'regular': 'blue', 'irregular': 'gray'}
            ...
            legend_elements = [Patch(facecolor=colors[k], label=k.replace('_', ' ').title()) 
                              for k in colors if k in patterns]
    ...
    def interpret_symbolic_meanings(self):
        ...
        if self.symmetry_score > 0.7:
            interpretations["geometric_summary"] = "High degree of geometric symmetry detected, suggesting deliberate design with potential symbolic meaning."
        elif self.symmetry_score > 0.5:
            interpretations["geometric_summary"] = "Moderate geometric symmetry detected, showing some intentional patterning."
        else:
            interpretations["geometric_summary"] = "Limited geometric symmetry, patterns may be coincidental or organic."
            
        # Add details about circles and triangles
        circle_count = len(self.geometric_centers)
        interpretations["geometric_summary"] += f"\nDetected approximately {circle_count} circular elements"
        
        # Interpret resonance patterns
        if hasattr(self, 'phase_relationships') and self.phase_relationships:
            harmonic_count = sum(1 for data in self.phase_relationships.values() 
                               if data["phase_pattern"] == "harmonic")
            golden_count = sum(1 for data in self.phase_relationships.values() 
                             if data["phase_pattern"] == "golden_ratio")
            
            if harmonic_count > len(self.phase_relationships) * 0.5:
                interpretations["resonance_summary"] = "Strong harmonic resonance patterns detected, suggesting intentional mathematical structuring."
            elif golden_count > len(self.phase_relationships) * 0.3:
                interpretations["resonance_summary"] = "Notable golden ratio alignments detected, suggesting sophisticated geometric knowledge."
            else:
                interpretations["resonance_summary"] = "Mixed resonance patterns with some significant frequency relationships."
                
            # Add details about specific resonances
            top_freqs = sorted(self.phase_relationships.items(), 
                              key=lambda x: x[1]["avg_strength"], reverse=True)[:3]
            
            interpretations["resonance_summary"] += "\nTop resonant frequencies:"
            for freq, data in top_freqs:
                interpretations["resonance_summary"] += f"\n- {freq:.3f} Hz ({data['phase_pattern']} pattern, strength: {data['avg_strength']:.2f})"
        else:
            interpretations["resonance_summary"] = "Limited resonance patterns detected."
            
        # Identify potential symbolic elements
        if self.symmetry_score > 0.6:
            interpretations["symbolic_elements"].append("Radial symmetry suggesting cosmic or universal symbolism")
            
        if circle_count >= 3:
            interpretations["symbolic_elements"].append("Circular elements suggesting cycles, completeness, or protection")
            
        if hasattr(self, 'phase_relationships'):
            harmonic_patterns = [p for p in self.phase_relationships.values() if p["phase_pattern"] == "harmonic"]
            if harmonic_patterns:
                interpretations["symbolic_elements"].append("Harmonic resonance patterns suggesting mathematical encoding")
                
            golden_patterns = [p for p in self.phase_relationships.values() if p["phase_pattern"] == "golden_ratio"]
            if golden_patterns:
                interpretations["symbolic_elements"].append("Golden ratio patterns suggesting advanced mathematical knowledge")
        ...
        if hasattr(self, 'phase_relationships') and self.phase_relationships:
            pattern_weights = {"harmonic": 1.0, "golden_ratio": 1.0, "regular": 0.7, "irregular": 0.3}
            weighted_sum = sum(pattern_weights[data["phase_pattern"]] * data["avg_strength"] 
                             for data in self.phase_relationships.values())
            resonance_rating = weighted_sum / len(self.phase_relationships) if self.phase_relationships else 0.0
            
        # Combine ratings
        interpretations["overall_rating"] = (geometric_rating * 0.5 + resonance_rating * 0.5) * 10
