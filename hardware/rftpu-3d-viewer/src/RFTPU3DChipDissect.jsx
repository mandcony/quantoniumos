...
// ===============================================
// RFT-PROCESSOR (Resonant Fourier Transform Processor)
...
export default function RFTPU3DChipDissect() {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (targetRotation) {
        ...
        if (Math.abs(rotation.x - targetRotation.x) < 0.01 && Math.abs(rotation.y - targetRotation.y) < 0.01) {
          setTargetRotation(null);
        }
      }
      if (targetZoom !== null) {
        setZoom(prev => prev + (targetZoom - prev) * 0.1);
        if (Math.abs(zoom - targetZoom) < 0.01) {
          setTargetZoom(null);
        }
      }
      if (targetExplode !== null) {
        setExplodeLevel(prev => prev + (targetExplode - prev) * 0.1);
        if (Math.abs(explodeLevel - targetExplode) < 0.01) {
  ...
  return (
    <div className="w-full h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-gray-950 relative overflow-hidden flex">
      ...
      {/* Right Panel - Enhanced Controls & Info */}
      <div className="w-80 bg-gray-900/95 backdrop-blur-xl border-l border-gray-700/50 flex flex-col overflow-hidden shadow-2xl">
        {/* View Controls */}
        ...
        <div className="p-4 border-b border-gray-700/50">
          <div className="text-sm text-cyan-400 font-mono font-bold mb-3 flex items-center gap-2">
            <span className="text-lg">👁</span> VISUAL MODE
          </div>
          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            {Object.entries(showLayers).map(([key, visible]) => (
              <button
                ...
                className={`px-2 py-1.5 text-xs font-mono rounded transition-all capitalize ${
                  visible 
        ...
        <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
          <div className="flex items-center justify-between mb-3">
            <div className="text-sm text-cyan-400 font-mono font-bold flex items-center gap-2">
              <span className="text-lg">📖</span>
              {selectedComponent ? EXPLANATIONS[selectedComponent].name : 'INFO'}
            </div>
            <button
              onClick={() => setSimpleMode(!simpleMode)}
              className={`px-3 py-1 text-xs font-mono rounded transition-all shadow-lg ${
                simpleMode 
          ...
          {selectedComponent ? (
            <div className="space-y-3">
              ...
              {EXPLANATIONS[selectedComponent].specs && (
                <div className="bg-cyan-900/20 border border-cyan-500/30 rounded-lg p-3">
                  <div className="text-xs text-cyan-400 font-mono font-bold mb-2">SPECIFICATIONS</div>
            ...
            <div className="text-sm text-gray-500 italic bg-gray-800/30 rounded-lg p-4 border border-gray-700 text-center">
        ...
        <div className="p-4 border-t border-gray-700/50">
          <div className="text-sm text-cyan-400 font-mono font-bold mb-3 flex items-center gap-2">
            <span className="text-lg">⏱️</span> CLOCK DOMAINS
          </div>
          <div className="space-y-1.5">
            {Object.entries(CLOCK_DOMAINS).map(([key, domain]) => (
              <div 
                key={key}
                className="flex items-center justify-between px-2 py-1.5 rounded bg-gray-800/50 border border-gray-700/50"
              >
                <div className="flex items-center gap-2">
                  <div 
                    ...
                    style={{ 
                      ...
                      boxShadow: `0 0 8px #${domain.color.toString(16).padStart(6, '0')}`
                    }}
                  />
                  <span className="text-xs text-gray-300 font-mono">{key}</span>
                </div>
                <span className="text-xs text-cyan-400 font-mono">{domain.freq} MHz</span>
              </div>
            ))}
          </div>
        </div>
        ...
        {/* Benchmark Scenarios */}
        <div className="p-4 border-t border-gray-700/50">
          <div className="text-sm text-purple-400 font-mono font-bold mb-3 flex items-center gap-2">
            <span className="text-lg">🚀</span> BENCHMARKS
          </div>
          <div className="space-y-1.5">
            {Object.entries(BENCHMARK_SCENARIOS).map(([key, scenario]) => (
              <button
                ...
                className={`w-full px-2 py-1.5 text-xs font-mono rounded transition-all text-left ${
        ...
        <div className="p-4 border-t border-gray-700/50 space-y-2">
          <div className="text-sm text-cyan-400 font-mono font-bold mb-2 flex items-center gap-2">
            <span className="text-lg">👁️</span> VISUALIZATION
          </div>
          <button
          ...
          >
            🌡️ Thermal View: {showThermalView ? 'ON' : 'OFF'}
          </button>
          <button
          ...
          >
            ⚡ Power Domains: {showPowerDomains ? 'ON' : 'OFF'}
          </button>
          <button
          ...
          >
            📈 Compare vs FFT: {showComparison ? 'ON' : 'OFF'}
          </button>
          <button
            onClick={() => setShowClockDomains(!showClockDomains)}
            className={`w-full px-3 py-2 text-xs font-mono rounded transition-all ${
              showClockDomains 
          ...
          >
            ⏱️ Clock Domains: {showClockDomains ? 'ON' : 'OFF'}
          </button>
          <button
          ...
          >
            📊 Metrics Panel: {showMetrics ? 'ON' : 'OFF'}
          </button>
          <button
