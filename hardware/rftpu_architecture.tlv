...
//
// Resonant Fourier Transform Processing Unit (RFTPU) Accelerator Architecture Blueprint
// ------------------------------------------------------------------------------------
// This TL-V file provides a synthesizable blueprint of the 64-tile RFTPU
// accelerator highlighted in the Resonant Fourier Transform Processing Unit documentation.
// Each tile wraps the Φ-RFT + SIS digest flow, supports kernel variant
// selection, exposes the tile registers described in the spec, and connects to
// a cycle-accurate NoC fabric model that emulates an 8x8 mesh with wormhole
// latency. The implementation focuses on architectural intent rather than
// numerical accuracy so that hardware and verification teams can iterate on the
// final microarchitecture without losing the documented structure.
...
\TLV
   ...
   $cycle[7:0] = $reset ? 8'd0 : >>1$cycle + 1'b1;

   $cfg_tile_id[6:0] = 7'd0;

   // CTRL_PAYLOAD_WIDTH = 128, DMA_PAYLOAD_WIDTH = 151, TILE_COUNT = 64
   $cfg_payload[127:0] =
      $reset ? 128'b0 :
      ($cycle == 8'd4) ? rftpu_pkg::make_ctrl_payload(
                           1'b1,      // start
                           4'h1,      // mode
                           16'd4,     // length (blocks)
                           32'd0,     // in_addr
   ...
   $block_counter[15:0] =
      $reset ? 16'd0 :
      ($dma_valid && *dma_ready) ? >>1$block_counter + 1'b1 :
      >>1$block_counter;

   $sample_seed[15:0] =
      $reset ? 16'd0 :
      ($dma_valid && *dma_ready) ? >>1$sample_seed + 16'd32 :
      >>1$sample_seed;

   // DMA_PAYLOAD_WIDTH = SAMPLE_FRAME_WIDTH(128) + 16 + 7 = 151
   $dma_payload[150:0] =
      $reset ? 151'b0 :
      ($dma_valid && *dma_ready) ? rftpu_pkg::make_dma_payload(
                                    rftpu_pkg::make_sample_frame(>>1$sample_seed),
                                    >>1$block_counter,
                                    7'd0
                                 ) :
      >>1$dma_payload;

   $cfg_ready_obs = cfg_ready;
   $dma_ready_obs = dma_ready;
   // TILE_COUNT = 64
   $tile_done_bitmap_obs[63:0] = tile_done_bitmap;
   $tile_busy_bitmap_obs[63:0] = tile_busy_bitmap;
   $global_irq_done_obs = global_irq_done;
   $global_irq_error_obs = global_irq_error;
   
   // Pass/fail conditions for Makerchip
   *passed = *cyc_cnt > 32'd100;
   *failed = 1'b0;
