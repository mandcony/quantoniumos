    ...
    class ResonanceEncryption:
        ...
        def encrypt(self, data, key):
            ...
            if isinstance(data, bytes):
                data = data.decode('utf-8', errors='surrogate')
            if isinstance(key, bytes):
                key = key.decode('utf-8', errors='surrogate')
            result = resonance_encrypt(data, key)
            return result.get('ciphertext', '').encode('utf-8')
        ...
        def decrypt(self, data, key):
            ...
            if isinstance(key, bytes):
                key = key.decode('utf-8', errors='surrogate')
            return decrypt_data(data, key).encode('utf-8')
except ImportError as e:
    print(f"Error importing QuantoniumOS modules: {e}")
    print("Make sure you're running this script from the project root or the correct modules are installed.")
    sys.exit(1)
            ...
            for func_name, results in self.results.items():
                if "_growth" not in func_name:
                    ...
                    if slope < 0.1:
                        ...
                        f.write("Memory growth pattern: **Quadratic (O(n²))**\n\n")
