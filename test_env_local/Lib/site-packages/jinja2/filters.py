...
def do_indent(
    ...
    :param first: Don't skip indenting the first line. :param blank: Don't skip indenting empty lines.
...
def do_wordwrap(
    ...
    # textwrap.wrap doesn't consider existing newlines when wrapping. # If the string has a newline before width, wrap will still insert # a newline at width, resulting in a short line. Instead, split and # wrap each paragraph individually. return wrapstring.join( [ wrapstring.join( textwrap.wrap( line, width=width, expand_tabs=False, replace_whitespace=False, break_long_words=break_long_words, break_on_hyphens=break_on_hyphens, ) ) for line in s.splitlines() ] ) _word_re = re.compile(r"\w+") def do_wordcount(s: str) -> int: """"""Count the words in that string."""""" return len(_word_re.findall(soft_str(s))) def do_int(value: t.Any, default: int = 0, base: int = 10) -> int: """"""Convert the value into an integer. If the conversion doesn't work it will return ``0``. You can
...
def do_attr(
    ...
    if the attribute doesn't exist. See :ref:`Notes on subscriptions <notes-on-subscriptions>` for more details. """""" # Environment.getattr will fall back to obj[name] if obj.name doesn't exist.
    # But we want to call env.getattr to get behavior such as sandboxing.
    # Determine if the attr exists first, so we know the fallback won't trigger. try: # This avoids executing properties/descriptors, but misses __getattr__ # and __getattribute__ dynamic attrs. getattr_static(obj, name) except AttributeError: # This finds dynamic attrs, and we know it's not a descriptor at this point.
