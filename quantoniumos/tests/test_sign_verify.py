...
def test_wave_hmac_sign_verify():
    ...
    assert not verify_wave_hmac(tampered_message, signature, key), "Tampered message was incorrectly verified"
    
    # Test with phase_info=False (pure HMAC)
    signature_pure = wave_hmac(message, key, phase_info=False)
    assert verify_wave_hmac(message, signature_pure, key, phase_info=False), "Pure HMAC verification failed"
