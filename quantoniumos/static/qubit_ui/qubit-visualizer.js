...
let oscillatorContext = null;
let container1Context = null;
let container2Context = null;
let frequency = 1.0;
let amplitude = 1.0;
...
function initializeQuantumGrid() {
    ...
    if (elements.gridFrequencySlider) {
        elements.gridFrequencySlider.addEventListener('input', updateFrequency);
    }
    
    // Initialize oscillator canvas
    oscillatorContext = elements.gridOscillatorCanvas.getContext('2d');
    
    // Initialize container schematics
    container1Context = elements.gridContainer1Canvas.getContext('2d');
    container2Context = elements.gridContainer2Canvas.getContext('2d');
    ...
    updateStatus("Quantum Grid initialized with 150-qubit support");
}
...
function updateQubitGrid(count) {
    ...
    updateStatus(`Quantum grid updated with ${gridQubitCount} qubits`);
}
...
function runQuantumGrid() {
    ...
    updateStatus(`Running quantum process on ${gridQubitCount} qubits...`);
...
function runStressTest() {
    updateStatus(`Running stress test with ${gridQubitCount} qubits...`);
    ...
    // Simulate delay for processing
    setTimeout(() => {
        simulateStressTestResults(gridQubitCount);
        // Update formula display for stress test
        updateFormulaDisplay(gridQubitCount, "Stress Test", true);
    }, 800);
}
...
function simulateQuantumResults(qubitCount, inputData) {
    ...
    updateStatus(`Quantum process completed for ${qubitCount} qubits`);
}
...
function simulateStressTestResults(qubitCount) {
    ...
    updateStatus(`Stress test completed for ${qubitCount} qubits (150-qubit capacity verified)`);
}
...
function updateFormulaDisplay(qubitCount, inputData, isStressTest = false) {
    ...
    if (isStressTest) {
        ...
        const formulas = [
            ...
            `φ = H^⊗${qubitCount} |${Array(qubitCount).fill('0').join('')}⟩`,
            `Σ = (1/√${2**qubitCount}) ⋅ ∑|x⟩`
        ];
        ...
        const formulas = [
            ...
            `H^⊗${Math.floor(qubitCount/2)} |${Array(Math.floor(qubitCount/2)).fill('0').join('')}⟩ = (1/√${2**Math.floor(qubitCount/2)}) ⋅ ∑|x⟩`
        ];
...
function startOscillatorAnimation() {
    if (!oscillatorContext || !elements.gridOscillatorCanvas) return;
    ...
    function animate() {
        ...
        for (let x = 0; x < width; x += step) {
            const y = height / 2 + Math.sin((x / width * 8 + time) * Math.PI * frequency) * amplitude * (height / 3);
            if (x === 0) {
                ctx.moveTo(x, y);
            } else {
                ctx.lineTo(x, y);
...
function updateFrequency() {
    ...
    frequency = parseFloat(elements.gridFrequencySlider.value);
    elements.gridFrequencyValue.textContent = frequency.toFixed(2) + ' Hz';
}
...
function drawContainerSchematics() {
    if (!container1Context || !container2Context) return;
    
    // Container 1: Regular pattern
    const ctx1 = container1Context;
