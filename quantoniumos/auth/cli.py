...
def create_key(args):
    ...
    with app_context():
        ...
        data = {
            ...
            "expires_at": format_datetime(key.expires_at) if key.expires_at else "Never",
...
def list_keys(args):
    ...
    with app_context():
        ...
        if args.json:
            ...
            headers = ["Key ID", "Prefix", "Name", "Admin", "Active", "Revoked", 
                      "Created", "Expires", "Last Used", "Use Count"]
            rows = [[d["key_id"], d["prefix"], d["name"], d["is_admin"], 
                   d["active"], d["revoked"], d["created"], d["expires"], 
                   d["last_used"], d["use_count"]] for d in data]
...
def rotate_key(args):
    ...
    with app_context():
        ...
        data = {
            ...
            "expires_at": format_datetime(new_key.expires_at) if new_key.expires_at else "Never",
...
def view_logs(args):
    ...
    with app_context():
        ...
        if args.json:
            ...
            headers = ["Timestamp", "Key ID", "Action", "IP Address", "Path", "Status", "Details"]
            rows = [[d["timestamp"], d["key_id"], d["action"], d["ip_address"], 
                   d["request_path"], d["status_code"], d["details"]] for d in data]
            print(tabulate(rows, headers=headers, tablefmt="pretty"))
...
def main():
    ...
    create_parser.add_argument("--expires-in-days", type=int, help="Key expiration in days")
    create_parser.add_argument("--permissions", default="api:read api:write", help="Space-separated permission list")
    create_parser.add_argument("--admin", action="store_true", help="Create an admin key with all permissions")
    ...
    # Rotate command
    rotate_parser = subparsers.add_parser("rotate", help="Rotate an API key (create new, deactivate old)")
    rotate_parser.add_argument("--id", required=True, help="ID of the key to rotate")
    rotate_parser.add_argument("--force", action="store_true", help="Skip confirmation prompt")
    rotate_parser.add_argument("--json", action="store_true", help="Output in JSON format")
    
    # Logs command
    logs_parser = subparsers.add_parser("logs", help="View audit logs")
    logs_parser.add_argument("--id", help="Filter logs by key ID")
    logs_parser.add_argument("--limit", type=int, default=50, help="Maximum number of logs to display")
