...
if not REDIS_DISABLED:
    try:
        ...
        logger.warning("Redis package not installed, running in no-Redis mode")
...
class QuantoniumRedisCluster:
    ...
    def __init__(self):
        ...
        if REDIS_DISABLED:
            logger.info("Redis support disabled via REDIS_DISABLED environment variable")
            return
    ...
    def connect(self) -> bool:
        ...
        # Skip if Redis is disabled
        if REDIS_DISABLED:
            logger.info("Redis connection skipped - running in no-Redis mode")
            return False
        ...
        try:
            ...
            # This occurs if Redis module is not imported
            logger.error(f"Redis module not available: {e}")
            return False
    ...
    def check_rate_limit(self, identifier: str, endpoint: str, limit: int, window: int) -> tuple:
        """
        Check rate limit using sliding window algorithm
        Returns (allowed: bool, remaining: int, reset_time: datetime)
        """
        # Skip rate limiting if Redis is disabled
        if REDIS_DISABLED or not self.redis_client:
    ...
    def log_security_event(self, event_data: Dict[str, Any]) -> bool:
        ...
        try:
            event_key = f"quantonium:security:events:{datetime.now().strftime('%Y%m%d')}"
        ...
        try:
            ...
            if profile_data:
                return {
                    ...
                    'endpoints_accessed': json.loads(profile_data.get('endpoints_accessed', '[]')),
                    'average_session_duration': float(profile_data.get('avg_session_duration', 0)),
    ...
    def update_user_behavior(self, user_id: str, endpoint: str, session_duration: float = 0) -> bool:
        ...
        try:
            ...
            current_endpoints = self.redis_client.hget(profile_key, 'endpoints_accessed')
            ...
            if session_duration > 0:
                current_avg = float(self.redis_client.hget(profile_key, 'avg_session_duration') or 0)
                request_count = int(self.redis_client.hget(profile_key, 'request_count') or 1)
                new_avg = ((current_avg * (request_count - 1)) + session_duration) / request_count
    ...
    def cache_quantum_computation(self, computation_id: str, result: Dict[str, Any], ttl: int = 3600) -> bool:
    ...
    def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            health_status['response_time_ms'] = (end_time - start_time).total_seconds() * 1000
...
# This is a simplified fallback that only works for a single instance
...
def get_redis_connection():
    ...
    if REDIS_DISABLED:
        logger.debug("Redis disabled, returning None for connection request")
        return None
...
def get_rate_limit(identifier: str, limit: int, window: int):
    ...
    if REDIS_DISABLED:
        ...
        if current_time > rate_limit_cache[key]["reset_at"]:
            rate_limit_cache[key] = {
                "count": 0,
                "reset_at": current_time + window
            }
        ...
        count = rate_limit_cache[key]["count"]
        reset_time = datetime.fromtimestamp(rate_limit_cache[key]["reset_at"])
        
        return count > limit, limit - count if count <= limit else 0, reset_time
    
    # Use Redis if available
    try:
        allowed, remaining, reset_time = redis_cluster.check_rate_limit(identifier, "general", limit, window)
        exceeded = not allowed
        return exceeded, remaining, reset_time
    except Exception as e:
        logger.error(f"Rate limit check failed: {e}")
        return False, limit, datetime.now()  # Fail open

# Configuration flags
REDIS_AVAILABLE = not REDIS_DISABLED and redis_cluster.redis_client is not None
