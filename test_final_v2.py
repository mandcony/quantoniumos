...
def test_enhanced_v2_performance():
    print("=== Enhanced RFT Crypto v2 - Final Performance Test ===")
    
    crypto = FixedRFTCryptoV2()
    
    # Test basic reversibility first
    key = secrets.token_bytes(32)
    plaintext = b"Hello, World! This is a comprehensive test message for our Enhanced RFT v2!"
    ...
    for size in test_sizes:
        ...
        encrypt_total = time.time() - start_time
        
        encrypt_throughput = (size * iterations) / (encrypt_total * 1024)  # KB/s
        
        target_met = "✓" if encrypt_throughput >= 30000 else "○"
    ...
    enc1 = crypto.encrypt(test_msg, key1)
    enc2 = crypto.encrypt(test_msg, bytes(key2))
    
    # Remove length headers for comparison
    enc1_data = enc1[4:]
    enc2_data = enc2[4:]
    
    diff_bits = 0
    total_bits = len(enc1_data) * 8
    
    for i in range(len(enc1_data)):
        diff_bits += bin(enc1_data[i] ^ enc2_data[i]).count('1')
    ...
    sensitive_keys = []
    base_encryption = crypto.encrypt(test_msg, key1)[4:]  # Remove header
    ...
    for bit_pos in range(min(256, len(key1) * 8)):  # Test first 256 bits
        ...
        test_encryption = crypto.encrypt(test_msg, bytes(test_key))[4:]  # Remove header
        
        # Count different bits
        diff_bits = 0
        for i in range(len(base_encryption)):
            diff_bits += bin(base_encryption[i] ^ test_encryption[i]).count('1')
    ...
    enc_msg1 = crypto.encrypt(test_message, key1)[4:]
    enc_msg2 = crypto.encrypt(bytes(test_message2), key1)[4:]
    
    diff_bits = 0
    for i in range(len(enc_msg1)):
        diff_bits += bin(enc_msg1[i] ^ enc_msg2[i]).count('1')
    ...
    overall_success = correct and avalanche_ok and sensitivity_ok and msg_avalanche_ok
    
    return overall_success

if __name__ == "__main__":
    success = test_enhanced_v2_performance()
