...
def verify_structure(extracted_path):
    ...
    required_dirs = ["core/HPC", "interface", "orchestration", "encryption"]
...
def integrate_modules(extracted_path, target_path="."):
    ...
    # Directories to copy
    dirs_to_copy = ["core/HPC", "interface", "orchestration", "encryption"]
    ...
    for dir_path in cpp_dirs:
        ...
        if os.path.isfile(cmake_file):
            ...
            try:
                ...
                subprocess.run(
                    ["cmake", ".."], check=True, text=True, capture_output=True
                )
    ...
    for dir_path in cpp_dirs:
        ...
        if os.path.isfile(setup_file):
            ...
            try:
                ...
                logger.error(
                    f"Pip install failed: {e.cmd}, Return code: {e.returncode}"
                )
                logger.error(f"Error output: {e.stderr}")
            os.chdir(base_path)
...
def main():
    ...
    try:
        ...
        if not verify_structure(temp_dir):
            logger.error(
                "Verification failed. The zip file does not contain the expected structure."
            )
            return 1
