...
Test with impulse input [1,0,0,0...] to isolate scaling factor
""""""
...
try:
 ...
 # Test with impulse (delta function)
 N = 16
 impulse = [1.0] + [0.0] * (N-1)

 print(f"Input: [1, 0, 0, 0, ...] (N={N})")
 print(f"Expected output energy: 1.0")

 rft = quantonium_core.ResonanceFourierTransform(impulse)
 output = rft.forward_transform()

 if isinstance(output[0], complex):
 energy = sum(abs(x)**2 for x in output)
 else:
 energy = sum(x*x for x in output)
