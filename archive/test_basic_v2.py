...
def test_basic_functionality():
    ...
    # Test with even-length input (Feistel requirement)
    key = secrets.token_bytes(32)
    plaintext = b"Hello, World! This is a test message!"  # 37 bytes -> will be padded to 38
...
def test_even_length():
    print("\n=== Testing with even-length input ===")
    
    cipher = enhanced_rft_crypto.PyEnhancedRFTCrypto()
    key = secrets.token_bytes(32)
    plaintext = b"Hello, World!123"  # 16 bytes (even)
    ...
    print(f"Decrypted ({len(decrypted)} bytes): {decrypted}")
