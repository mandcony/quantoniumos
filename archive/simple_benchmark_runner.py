#!/usr/bin/env python3
""""""
Simple RFT Benchmark Runner - No Fluff Edition

Run individual benchmarks to validate RFT paradigm with fair comparisons:
- Crypto: Avalanche/differential score (RFT vs AES)
- Quantum: Coherence ratio vs baseline
- Optimization: Rosenbrock minima (RFT vs standard)
- Pattern: Silhouette score difference on resonance patterns
""""""
...
def run_benchmark(name, scale="small"):
    """"""Run a single benchmark and return the key validation metric""""""

    benchmarks = {
        "crypto": "crypto_benchmark.py",
        "quantum": "quantum_benchmark.py",
        "optimization": "optimization_benchmark.py",
        "pattern": "pattern_benchmark.py"
    }
    ...
    try:
        # Simple subprocess call - no complex argument parsing
        ...
        # Load and extract the key metric
        results_file = f"{name}_results.json"
        if Path(results_file).exists():
            with open(results_file) as f:
                data = json.load(f)
            return extract_key_metric(name, data)
...
def validate_rft_paradigm():
    """"""Run all benchmarks and check if RFT shows consistent improvement""""""

    print("🚀 RFT PARADIGM VALIDATION")
    print("=" * 40)

    benchmarks = ["crypto", "quantum", "optimization", "pattern"]
    results = {}

    for benchmark in benchmarks:
        metric = run_benchmark(benchmark, scale="small")
    ...
    # Final validation
    successful = sum(1 for m in results.values() if m and m > 1.0)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        run_benchmark(benchmark, scale)
    else:
        # Run validation suite
        validate_rft_paradigm()
