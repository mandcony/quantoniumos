...
def test_no_dft_in_main_module():
    ...
    with open('/workspaces/quantoniumos/core/encryption/resonance_fourier.py', 'r') as f:
        content = f.read()
...
# Legacy wrapper maintained for: resonance_fourier_transform, inverse_resonance_fourier_transform
        ...
        # Test signal
        signal = [1.0, 2.0, 3.0, 4.0, 0.5, 1.5]

        # Forward transform
        forward_result = resonance_fourier_transform(signal)
        print(f"✅ Forward RFT: {len(forward_result)} frequency components")

        # Inverse transform
        inverse_result = inverse_resonance_fourier_transform(forward_result)
        print(f"✅ Inverse RFT: reconstructed {len(inverse_result)} samples")

        # Check reconstruction quality
        if len(inverse_result) >= len(signal):
            error = np.mean(np.abs(np.array(inverse_result[:len(signal)]) - np.array(signal)))
...
def test_duplicate_files_removed():
    ...
    # Find all resonance_fourier files
    patterns = [
        '/workspaces/quantoniumos/**/resonance_fourier_*.py',
        '/workspaces/quantoniumos/**/resonance_fourier_old.py',
        '/workspaces/quantoniumos/**/resonance_fourier_clean.py'
    ]
    ...
    # Filter out test_env_local (that's expected) duplicates = [f for f in duplicates if 'test_env_local' not in f]
...
if __name__ == "__main__":
    ...
    results = []
    results.append(test_no_dft_in_main_module())
    results.append(test_true_rft_works())
    results.append(test_duplicate_files_removed())
