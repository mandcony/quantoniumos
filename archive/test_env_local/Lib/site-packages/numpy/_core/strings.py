...
def expandtabs(a, tabsize=8):
    ...
    >>> b = np.array(['hello world', 'γεια sigmaου κόsigmamuε', '你好世界', '👋 🌍'],
    ...              dtype=np.dtypes.StringDType())
    >>> np.strings.slice(b, -2)
    array(['hello wor', 'γεια sigmaου κόsigma', '你好', '👋'], dtype=StringDType())

    >>> np.strings.slice(b, [3, -10, 2, -3], [-1, -2, -1, 3])
    array(['lo worl', ' sigmaου κόsigma', '世', '👋 🌍'], dtype=StringDType())

    >>> np.strings.slice(b, None, None, -1)
    array(['dlrow olleh', 'εmusigmaόκ υοsigma αιεγ', '界世好你', '🌍 👋'],
          dtype=StringDType())
