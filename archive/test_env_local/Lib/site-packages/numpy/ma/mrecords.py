...
class MaskedRecords(ma.MaskedArray):
    ...
    def __getattribute__(self, attr):
        ...
        # Reset the object's mask hasmasked = False _mask = _localdict.get('_mask', None) if _mask is not None: try: _mask = _mask[attr] except IndexError: # Couldn't find a mask: use the default (nomask)
                pass
    ...
    def __getitem__(self, indx):
        ...
        if isinstance(indx, str):
            ...
            # Don't use _set_mask, there are some copies being made that # break propagation Don't force the mask to nomask, that wreaks
            # easy masking
        ...
        # Here again.
            ...
            try:
                if issubclass(dtype, np.ndarray):
                    ...
                    output = np.ndarray.view(self, dtype)
            # OK, there's the change except TypeError: dtype = np.dtype(dtype) # we need to revert to MaskedArray, but keeping the possibility # of subclasses (eg, TimeSeriesRecords), so we'll force a type
