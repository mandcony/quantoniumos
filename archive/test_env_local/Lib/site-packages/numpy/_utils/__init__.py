...
def _rename_parameter(old_names, new_names, dep_version=None):
    ...
    Notes
    -----
    Untested with functions that accept *args. Probably won't work as written. """""" def decorator(fun): @functools.wraps(fun) def wrapper(*args, **kwargs): __tracebackhide__ = True # Hide traceback for py.test for old_name, new_name in zip(old_names, new_names): if old_name in kwargs: if dep_version: end_version = dep_version.split('.') end_version[1] = str(int(end_version[1]) + 2) end_version = '.'.join(end_version)
