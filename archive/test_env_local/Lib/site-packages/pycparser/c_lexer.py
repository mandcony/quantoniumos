...
class CLexer(object):
    ...
    def __init__(self, error_func, on_lbrace_func, on_rbrace_func,
        ...
        """""" Create a new Lexer.
            ...
            on_lbrace_func, on_rbrace_func:
                ...
                (likely to push/pop type_lookup_func's scope) type_lookup_func: A type lookup function. Given a string, it must return True IFF this string is a name of a type that was defined with a typedef earlier. """""" self.error_func = error_func self.on_lbrace_func = on_lbrace_func self.on_rbrace_func = on_rbrace_func self.type_lookup_func = type_lookup_func self.filename = '' # Keeps track of the last token returned from self.token() self.last_token = None # Allow either "# line" or "# <num>" to support GCC's
        # cpp output
        #
    ...
    #
    #   simple_escape = r""""""([a-zA-Z._~!=&\^\-\\?'"])"""""" # decimal_escape = r""""""(\d+)"""""" # hex_escape = r""""""(x[0-9a-fA-F]+)"""""" # bad_escape = r""""""([\\][^a-zA-Z._~^!=&\^\-\\?'"x0-7])""""""
    #
    ...
    # with the braces) it'sa parameter. The trouble begins with yacc's
