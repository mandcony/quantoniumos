...
class TestLayout:
    ...
    def test_smoke_initial_pos_forceatlas2(self):
        ...
        nx.forceatlas2_layout(self.Gi, pos=pos)

    def test_smoke_initial_pos_fruchterman_reingold(self):
        pos = nx.circular_layout(self.Gi)
        nx.fruchterman_reingold_layout(self.Gi, pos=pos)

    def test_smoke_initial_pos_arf(self):
        pos = nx.circular_layout(self.Gi)
        nx.arf_layout(self.Gi, pos=pos)
    ...
    def test_node_at_center(self):
        ...
        nx.forceatlas2_layout(G, pos=orig_pos)

    def test_initial_only_some_pos(self):
        G = nx.path_graph(3)
        orig_pos = {i: [i - 1, 0.0] for i in range(2)}
        nx.forceatlas2_layout(G, pos=orig_pos, seed=42)
...
def test_bipartite_layout_default_nodes_raises_non_bipartite_input():
    ...
    nx.bipartite_layout(G, nodes=[2, 3])
