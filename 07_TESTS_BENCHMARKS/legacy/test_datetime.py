...
class TestDateTime:
    ...
    @pytest.mark.parametrize("val1, val2", [
        # the smallest integer that can't be represented # exactly in a double should be preserved if we avoid # casting to double in floordiv operation (9007199254740993, 1), # stress the alternate floordiv code path where # operand signs don't match and remainder isn't 0 (9007199254740999, -2), ]) def test_timedelta_floor_div_precision(self, val1, val2): op1 = np.timedelta64(val1) op2 = np.timedelta64(val2) actual = op1 // op2 # Python reference integer floor expected = val1 // val2 assert_equal(actual, expected) @pytest.mark.parametrize("val1, val2", [ # years and months sometimes can't be unambiguously
    ...
    def test_datetime_as_string(self):
        ...
        for us in ['us', 'mus', b'us']:  # check non-ascii and bytes too
        ...
        assert_equal(np.datetime_as_string(
                ...
                '2032-07-18T12:23')
        # 'auto' doesn't split up hour and minute assert_equal(np.datetime_as_string( np.datetime64('2032-07-18T12:00:00', 'us'), unit='auto'), '2032-07-18T12:00') assert_equal(np.datetime_as_string( np.datetime64('2032-07-18T00:00:00', 'us'), unit='auto'), '2032-07-18') # 'auto' doesn't split up the date
        assert_equal(np.datetime_as_string(
    ...
    def test_timedelta_hash_weeks(self, unit):
        ...
        assert hash(td) != hash(td3)  # doesn't collide @pytest.mark.parametrize('unit', ('W', 'D', 'h', 'm', 's', 'ms', 'us')) def test_timedelta_hash_weeks_vs_pydelta(self, unit): td = np.timedelta64(10, 'W') td2 = np.timedelta64(td, unit) pytd = td2.astype(datetime.timedelta) assert isinstance(pytd, datetime.timedelta) _assert_equal_hash(pytd, td2) @pytest.mark.parametrize('unit', ('ms', 'us', 'ns', 'ps', 'fs', 'as')) def test_timedelta_hash_ms(self, unit): td = np.timedelta64(3, 'ms') td2 = np.timedelta64(td, unit) _assert_equal_hash(td, td2) td3 = np.timedelta64(int(td2.astype(int)) + 1, unit) assert hash(td) != hash(td3) # doesn't collide
...
class TestDateTimeData:
    ...
    def test_non_ascii(self):
        # mus is normalized to mu
        dt = np.datetime64('2000', ('mus', 5))
        assert np.datetime_data(dt.dtype) == ('us', 5)

        dt = np.datetime64('2000', '5mus')
        assert np.datetime_data(dt.dtype) == ('us', 5)
