...
def test_figure_clear(clear_meth):
    ...
    [fig.add_subplot(2, 1, i + 1) for i in range(2)]
    getattr(fig, clear_meth)()
    assert fig.axes == []
...
class TestSubplotMosaic:
    ...
    def test_subplot_kw(self, fig_test, fig_ref, subplot_kw):
        x = [[1, 2]]
        fig_test.subplot_mosaic(x, subplot_kw=subplot_kw)
        subplot_kw = subplot_kw or {}

        gs = fig_ref.add_gridspec(1, 2)
        fig_ref.add_subplot(gs[0, 0], **subplot_kw)

        fig_ref.add_subplot(gs[0, 1], **subplot_kw)
...
def test_subfigure_ticks():
    ...
    subfig_bl.add_subplot(gs[0, 3:14], sharey=ax1)
