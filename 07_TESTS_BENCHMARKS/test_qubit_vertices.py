...
class QubitVertexNetwork:
    ...
    def evolve_network(self, time_steps: int = 5, dt: float = 0.1):
        ...
        for step in range(time_steps):
            ...
            if step % 2 == 0:
                total_p1 = sum(
                    v.measure_probabilities()[1] for v in self.vertices.values()
                )
                avg_phase = np.mean([v.phase for v in self.vertices.values()])
                print(
                    f"  Step {step}: total_P(1)={total_p1:.3f}, avg_phase={avg_phase:.3f}"
                )
    ...
    def get_network_state(self) -> Dict[str, Any]:
        ...
        superposition_count = sum(
            1 for state in vertex_states if state["is_superposition"]
        )
        ...
        return {
            ...
            "connectivity": self.graph.number_of_edges()
...
def test_qubit_vertices():
    ...
    for i in range(min(4, network.num_vertices)):  # Show first 4 vertices
        ...
        print(
            f"  Vertex {i}: P(0)={state['prob_0']:.3f}, P(1)={state['prob_1']:.3f}, "
