...
class TestMethods:
    ...
    @pytest.mark.parametrize("buf,old,new,count,res", [
        ...
        ("Who goes there??", "?", "!", -1, "Who goes there!!"),
        ("Who goes there?", ".", "!", -1, "Who goes there?"),
        ("This is a tissue", "is", "**", -1, "Th** ** at**sue"),
        ("This is a tissue", "is", "**", -1, "Th** ** at**sue"),
        ("This is a tissue", "is", "**", 4, "Th** ** at**sue"),
        ("This is a tissue", "is", "**", 3, "Th** ** at**sue"),
...
class TestMethodsWithUnicode:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            ("lambdamu", "lambda"),
            ("lambda" * 5 + "mu" * 2, "mu"),
            ("lambda" * 5 + "mu" * 2, "lambda"),
            ("lambda" * 5 + "A" + "mu" * 2, "mulambda"),
            ("lambdamu" * 5, "mu"),
            ("lambdamu" * 5, "lambda"),
    ])
...
class TestMixedTypeMethods:
    def test_center(self):
        ...
        with pytest.raises(ValueError, match="'ascii' codec can't encode"): buf = np.array("s", dtype="S") fill = np.array("😊", dtype="U") np.strings.center(buf, 3, fill) def test_ljust(self): buf = np.array("😊", dtype="U") fill = np.array("*", dtype="S") res = np.array("😊**", dtype="U") assert_array_equal(np.strings.ljust(buf, 3, fill), res) buf = np.array("s", dtype="S") fill = np.array("*", dtype="U") res = np.array("s**", dtype="S") assert_array_equal(np.strings.ljust(buf, 3, fill), res) with pytest.raises(ValueError, match="'ascii' codec can't encode"):
            buf = np.array("s", dtype="S")
            fill = np.array("😊", dtype="U")
