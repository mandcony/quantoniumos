...
AND formal cryptographic security proofs and experiments. Updated to include
rigorous security testing beyond just functional validation.
"""
...
def main():
    ...
    for test_file, description in security_tests:
        if os.path.exists(test_file):
            if test_file.endswith(".py") and (
                "quantum_proofs" in test_file or "formal_proofs" in test_file
            ):
                # These are modules, not test scripts - run as modules
                test_results[test_file] = run_python_module_directly(
    ...
    security_test_files = [
        f
        for f in test_results.keys()
        if "security" in f or "collision" in f or "quantum" in f or "formal" in f
    ]
    ...
    statistical_test_files = [
        f
        for f in test_results.keys()
        if "avalanche" in f or "statistical" in f or "resonance" in f
    ]
    statistical_passed = sum(
        1 for f in statistical_test_files if test_results.get(f, False)
    )
