    ...
    def test_ignore_object_identity_in_equal(self):
        ...
        class FunkyType:
            def __eq__(self, other):
                raise TypeError("I won't compare") a = np.array([FunkyType()]) assert_raises(TypeError, np.equal, a, a) # Check identity doesn't override comparison mismatch.
    ...
    def test_ignore_object_identity_in_not_equal(self):
        ...
        class FunkyType:
            def __ne__(self, other):
                raise TypeError("I won't compare") a = np.array([FunkyType()]) assert_raises(TypeError, np.not_equal, a, a) # Check identity doesn't override comparison mismatch.
        a = np.array([np.nan], dtype=object)
...
class TestLog:
    ...
    def test_log_values_maxofdtype(self):
        ...
        # other non x86-64 platforms checking `longdouble` isn't too useful as # it's an alias for float64.
...
class TestRationalFunctions:
    ...
    def test_lcm_overflow(self):
        # verify that we don't overflow when a*b does overflow big = np.int32(np.iinfo(np.int32).max // 11) a = 2 * big b = 5 * big assert_equal(np.lcm(a, b), 10 * big) def test_gcd_overflow(self): for dtype in (np.int32, np.int64): # verify that we don't overflow when taking abs(x)
...
class TestComplexFunctions:
    ...
    @pytest.mark.xfail(IS_WASM, reason="doesn't work") def test_precisions_consistent(self): z = 1 + 1j for f in self.funcs: fcf = f(np.csingle(z)) fcd = f(np.cdouble(z)) fcl = f(np.clongdouble(z)) assert_almost_equal(fcf, fcd, decimal=6, err_msg=f'fch-fcd {f}') assert_almost_equal(fcl, fcd, decimal=15, err_msg=f'fch-fcl {f}') @pytest.mark.xfail(IS_WASM, reason="doesn't work")
