...
def evaluate_shift_diagonalization(n: int) -> Dict:
    """
...
def run_shift_diagonalization_test() -> Dict:
    ...
    for n in sizes:
        test_result = evaluate_shift_diagonalization(n)
...
def test_shift_diagonalization_behavior():
    """RFT should fail to diagonalize the shift operator while DFT succeeds."""
    result = evaluate_shift_diagonalization(32)
    assert result['dft_diagonalizes'], "DFT should diagonalize cyclic shift"
    assert result['rft_fails_to_diagonalize'], "RFT should leave ≥10% off-diagonal energy"
