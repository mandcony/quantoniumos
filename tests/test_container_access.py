...
from core.encryption.resonance_encrypt import (decrypt_symbolic,
                                               encrypt_symbolic)
...
def test_container_creation_and_access():
    ...
    for container in containers:
        ...
        assert (
            access_granted
        ), f"Access validation failed for hash {container['hash'][:8]}..."

        # Get the container and check its contents
        stored_container = get_container_by_hash(container["hash"])
        assert (
            stored_container is not None
        ), f"Container not found for hash {container['hash'][:8]}..."
        assert (
            stored_container["plaintext"] == container["plaintext"]
        ), "Plaintext mismatch"

        print(
            f"  Successfully accessed container with hash: {container['hash'][:8]}..."
        )
...
def test_container_collision_resistance():
    ...
    for plaintext in plaintexts:
        ...
        print(
            f"  Created container with plaintext: {plaintext[:8]}... hash: {result['hash'][:8]}..."
        )
    ...
    for i, plaintext in enumerate(plaintexts):
        for j, hash_value in enumerate(hashes):
            ...
            if i == j:
                ...
                ), f"Plaintext mismatch for hash {hash_value[:8]}..."
            else:
                # Wrong hash for this plaintext
                assert (
                    container["plaintext"] != plaintext
                ), f"Incorrect plaintext match for hash {hash_value[:8]}..."
