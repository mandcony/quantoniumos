...
try:
    from encryption.resonance_fourier import (
        inverse_resonance_fourier_transform, perform_irft_list,
        perform_rft_list, resonance_fourier_transform)
...
class RFTRoundTripTest:
    ...
    def generate_test_signals(self) -> List[Tuple[str, List[float]]]:
        ...
        complex_wave = [
            math.sin(2 * math.pi * i / 8) + 0.5 * math.cos(2 * math.pi * i / 4)
            for i in range(16)
        ]
    ...
    def test_parseval_theorem(self):
        ...
        for signal_name, signal in test_signals:
            try:
                ...
                freq_energy = sum(
                    abs(complex_val) ** 2 for _, complex_val in frequency_components
                )
                ...
                self.test_results.append(
                    {
                        ...
                        "reason": (
                            "Parseval theorem implementation pending; see issue #43"
                            if not passed
                            else None
                        ),
    ...
    def test_linearity_property(self):
        ...
        try:
            ...
            rft_linear = [
                (f1, a * c1 + b * c2)
                for (f1, c1), (f2, c2) in zip(rft_signal1, rft_signal2)
            ]

            # Compare results
            max_diff = max(
                abs(c1 - c2) for (_, c1), (_, c2) in zip(rft_combined, rft_linear)
            )
            ...
            self.test_results.append(
                {"test": "linearity_property", "status": "ERROR", "error": str(e)}
            )
    ...
    def run_all_tests(self):
        ...
        skipped_tests = sum(
            1 for result in self.test_results if result["status"] == "SKIPPED"
        )
        xfailed_tests = sum(
            1 for result in self.test_results if result["status"] == "XFAIL"
        )
        ...
        for result in self.test_results:
            ...
            if status == "PASS" and "mse" in result:
                ...
                print(
                    f"    MSE: {result['mse']:.2e} (tolerance: {result['tolerance']:.2e})"
                )
            elif status == "ERROR":
