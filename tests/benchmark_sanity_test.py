...
def run_sanity_test() -> bool:
    ...
    try:
        response = requests.post(f"{BASE_URL}/api/benchmark", 
                                json={"plaintext": plaintext, "key": key})
        response.raise_for_status()
        result = response.json()
        
        logger.info(f"Benchmark API responded with status: {result.get('status', 'unknown')}")
        ...
        if rows_written < 60:  # Allow for some flexibility but should be close to 64
            logger.error(f"Expected approximately 64 perturbation tests, got {rows_written}")
            return False
        ...
        encryption_response = requests.post(f"{BASE_URL}/api/encrypt", 
                                        json={"plaintext": plaintext, "key": key})
        ...
        encrypt_resp = requests.post(f"{BASE_URL}/api/encrypt", 
                                   json={"plaintext": plaintext, "key": key})
...
def check_database_security() -> bool:
    ...
    try:
        ...
        if response.status_code in (401, 403, 404):
            ...
            logger.warning(f"Potential security concern: API keys endpoint returned {response.status_code}")
            return False
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if not benchmark_result or not security_check:
        ...
        if not benchmark_result:
            print("- Fix the 64-perturbation benchmark to ensure all tests run properly")
        if not security_check:
            print("- SECURITY ISSUE: Update database permissions to restrict access to API key tables")
            print("- Consider changing schema from 'public' to 'secure' for sensitive tables")
