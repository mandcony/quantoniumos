...
def test_collision_analysis(num_samples=1000):
    ...
    results = {
        ...
        "collision_details": {hash_val: glyphs for hash_val, glyphs in collisions.items()}
    }
    
    logger.info(f"Collision analysis complete: {total_unique} unique hashes, {total_collisions} collisions")
    return results
...
def avalanche_analysis(num_samples=1000, bit_flips=1):
    ...
    logger.info(f"Starting avalanche analysis with {num_samples} samples, {bit_flips} bit flips")
    ...
    for i in range(num_samples):
        ...
        modified_bytes = bytes(int(modified_bits[i:i+8], 2) for i in range(0, len(modified_bits), 8))
        
        # Get the modified hash
        modified_hash = geometric_hash(modified_bytes)
        
        # Compare the hashes bit by bit
        original_hash_bits = ''.join(format(b, '08b') for b in original_hash) if isinstance(original_hash, bytes) else bin(int(original_hash, 16))[2:].zfill(len(original_hash) * 4)
        modified_hash_bits = ''.join(format(b, '08b') for b in modified_hash) if isinstance(modified_hash, bytes) else bin(int(modified_hash, 16))[2:].zfill(len(modified_hash) * 4)
        ...
        hamming_distance = sum(o != m for o, m in zip(original_hash_bits, modified_hash_bits))
    ...
    if total_bits_compared > 0:
        results["bit_change_percentage"] = (total_bits_changed / total_bits_compared) * 100
    
    # Calculate expected ideal avalanche effect (50% bit change)
    results["ideal_bit_change"] = 50.0
    results["avalanche_quality"] = 100 - abs(results["bit_change_percentage"] - results["ideal_bit_change"])
    
    logger.info(f"Avalanche analysis complete: {results['bit_change_percentage']:.2f}% bits changed on average")
    return results
...
def plot_collision_results(results, save_path=None):
    ...
    # Basic stats
    ax.bar(["Total Samples", "Unique Hashes"], 
    ...
    ax.text(0.5, 0.9, collision_text, transform=ax.transAxes, ha='center', va='center', 
...
def plot_avalanche_results(results, save_path=None):
    ...
    values = [results["bit_change_percentage"], results["ideal_bit_change"], results["avalanche_quality"]]
    ...
    expected_bits = len(results["hamming_distances"][0]) // 2 if results["hamming_distances"] else 0
    if expected_bits > 0:
