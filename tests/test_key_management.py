...
API_KEY = os.environ.get(
    "QUANTONIUM_API_KEY", "test_key_for_testing"
)  # Default for testing
...
def get_auth_token():
    ...
    if not API_KEY:
        pytest.skip(
            "No API key found. Set the QUANTONIUM_API_KEY environment variable."
        )
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/auth/token", headers=auth_headers, timeout=1
        )

        if response.status_code != 200:
            pytest.skip(
                f"API server not available or auth failed: {response.status_code}"
            )
...
def test_create_api_key():
    ...
    try:
        response = requests.post(
            f"{BASE_URL}/api/auth/keys", headers=get_headers(), json=payload, timeout=5
        )
...
def test_list_api_keys():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/auth/keys", headers=get_headers(), timeout=5
        )
...
def test_get_profile():
    ...
    try:
        response = requests.get(
            f"{BASE_URL}/api/auth/profile", headers=get_headers(), timeout=5
        )
...
class TestKeyManagementIntegration:
    ...
    def test_integration_flow(self):
        ...
        try:
            ...
            response = requests.get(
                f"{BASE_URL}/api/auth/profile", headers=get_headers(), timeout=1
            )
            if response.status_code != 200:
