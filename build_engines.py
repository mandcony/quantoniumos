...
This script builds all C++ engines using pybind11 and makes them available
to Python with automatic fallback handling.
"""
...
def check_dependencies():
    ...
    if not cpp_found:
        required.append('C++ compiler (g++/clang++/MSVC)')
    ...
    eigen_paths = [
        ...
        'third_party/eigen',
        Path.cwd() / 'third_party' / 'eigen'
    ]
    
    eigen_found = False
    for path in eigen_paths:
        if Path(path).exists() and (Path(path) / 'Eigen').exists():
            print(f"✓ Eigen3 found: {path}")
            eigen_found = True
            break
    
    if not eigen_found:
        required.append('eigen3')
...
def build_with_cmake():
    ...
    build_result = subprocess.run(['cmake', '--build', '.', '--config', 'Release'], 
...
def build_with_setuptools_fallback():
    ...
    extensions = [
        Pybind11Extension(
            "quantonium_core",
            ["core/pybind_interface.cpp", "core/engine_core.cpp", "core/symbolic_eigenvector.cpp"],
            include_dirs=[
        ...
        Pybind11Extension(
            "resonance_engine", 
            ["core/resonance_engine_bindings.cpp", "core/engine_core.cpp", "core/symbolic_eigenvector.cpp"],
            include_dirs=[
                pybind11.get_cmake_dir(),
                "core/include", 
                "third_party/eigen"
        ...
        Pybind11Extension(
            "quantum_engine",
            ["core/quantum_engine_bindings.cpp", "core/engine_core.cpp", "core/symbolic_eigenvector.cpp"],
            include_dirs=[
                pybind11.get_cmake_dir(),
                "core/include",
                "third_party/eigen"
...
def install_modules():
    ...
    modules = ['quantonium_core', 'resonance_engine', 'quantum_engine']
    ...
    for module in modules:
        ...
        for ext in possible_extensions:
            ...
            build_path = Path('build') / f"{module}{ext}"
            if build_path.exists():
                # Copy to core directory
                dest_path = Path('core') / f"{module}{ext}"
                shutil.copy2(build_path, dest_path)
                print(f"✓ Installed {module}{ext}")
                found = True
                installed_count += 1
                break
            
            # Check in current directory (setuptools build)
            current_path = Path(f"{module}{ext}")
...
def test_engines():
    ...
    try:
        ...
        engine = resonance_engine.ResonanceFourierEngine()
        result = engine.forward_true_rft([1.0, 0.5, -0.3, 0.8])
        test_results['resonance_engine'] = f"✓ Working ({len(result)} coefficients)"
    except Exception as e:
        test_results['resonance_engine'] = f"❌ Failed: {e}"
    
    # Test quantum engine
    try:
        import quantum_engine
        qe = quantum_engine.QuantumEntropyEngine()
        entropy = qe.generate_quantum_entropy(10)
        test_results['quantum_engine'] = f"✓ Working ({len(entropy)} entropy values)"
    except Exception as e:
        test_results['quantum_engine'] = f"❌ Failed: {e}"
    
    # Test core engine
    try:
        import quantonium_core
        rft = quantonium_core.ResonanceFourierTransform([1.0, 0.5, -0.3, 0.8])
        freq = rft.forward_transform()
        test_results['quantonium_core'] = f"✓ Working ({len(freq)} frequencies)"
    except Exception as e:
        test_results['quantonium_core'] = f"❌ Failed: {e}"
    
    print("Test Results:")
    for engine, result in test_results.items():
        print(f"  {engine}: {result}")
    
    working_engines = sum(1 for result in test_results.values() if result.startswith('✓'))
...
def main():
    ...
    if shutil.which('cmake'):
        build_success = build_with_cmake()
        if build_success:
            os.chdir('..')  # Return to root directory
    
    # Fallback to setuptools if CMake failed
    if not build_success:
        print("CMake build failed, trying setuptools fallback...")
        try:
            build_success = build_with_setuptools_fallback()
        except Exception as e:
    ...
    if test_engines():
        ...
        print("\nNext steps:")
        print("1. Run: python -c \"from core.high_performance_engine import get_engine_status; print(get_engine_status())\"")
        print("2. Your Python code will now automatically use high-performance C++ implementations")
    else:
        print("\n⚠️ Build completed but engines have issues.")
        print("The system will fall back to Python implementations.")
