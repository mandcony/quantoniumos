...
- Clean card layout ΓÇó Light/Dark toggle ΓÇó Pause/Resume
...
class SystemMonitor(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("QuantoniumOS ΓÇó System Monitor")
    ...
    def _build_ui(self):
        ...
        subtitle = QLabel("Live metrics ΓÇó minimal visuals ΓÇó Quantonium aesthetic")
        ...
        self.search = QLineEdit(placeholderText="Search processesΓÇª")
        ...
        self.disk_label = QLabel("ΓÇö"); self.disk_label.setObjectName("Mid")
        self.disk_bar = Bar()
        ...
        self.net_up = QLabel("Γåæ 0 B/s"); self.net_dn = QLabel("Γåô 0 B/s")
        ...
        self.rft_status = QLabel("≡ƒƒó Online" if RFT_AVAILABLE else "≡ƒö┤ Offline")
        ...
        self.rft_viz_btn = QPushButton("≡ƒîè RFT Visualizer")
    ...
    def _status_text(self) -> str:
        ...
        return f"{osname} ΓÇó RFT {'Online' if RFT_AVAILABLE else 'Offline'}"
    ...
    def _update_net(self):
        ...
        if self._last_net:
            ...
            self.net_up.setText(f"Γåæ {human_rate(up)}")
            self.net_dn.setText(f"Γåô {human_rate(dn)}")
            self.net_up_spark.add(min(100.0, 100.0*(up/(1024*1024*10))))  # normalize to 10 MB/s Γëê 100%
...
class RFTVisualizerDialog(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        # Timer (do NOT start yetΓÇöavoid race before canvas exists)
        self.timer = QTimer(self)
    ...
    def init_ui(self):
        ...
        if HAS_MPL:
            ...
            fallback = QLabel("Matplotlib not available ΓÇö 3D view disabled.")
    ...
    def update_metrics(self, wave):
        ...
        metrics_text = f"""
≡ƒîè QUANTUM FIELD ANALYSIS
ΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöüΓöü

≡ƒôè WAVE PROPERTIES:
   ΓÇó Amplitude Range: {arange:.3f}
   ΓÇó Peak Intensity: {amax:.3f}
   ΓÇó Field Average: {mean:.3f}
   ΓÇó Complexity: {complexity}%
...
ΓÜ¢∩╕Å QUANTUM PARAMETERS:
   ...
   ΓÇó Base Frequency: {self.frequency:.1f} Hz
   ΓÇó Quantum Coupling: {self.quantum_coupling:.3f}
   ΓÇó Time Evolution: {self.time_step:.1f}s

≡ƒôê FIELD EXPLANATION:
   ΓÇó Higher complexity = more intricate patterns
   ΓÇó Coherence shows wave synchronization
   ΓÇó Interference reveals wave interactions
   ΓÇó Phase sync indicates temporal stability

≡ƒö¼ OBSERVED EFFECTS:
   ΓÇó Recursive layers create depth
   ΓÇó Quantum coupling adds nonlinearity
   ΓÇó Interference generates complexity
   ΓÇó Time evolution shows dynamics
"""
