...
Remove all Unicode emoji characters from Python files in the project
""""""
...
def main():
    ...
    for py_file in python_files:
        try:
            emojis_removed = remove_emojis_from_file(py_file)

            if emojis_removed > 0:
                rel_path = py_file.relative_to(project_root)
                print(f" {rel_path}: {emojis_removed} emojis removed")
    ...
    for md_file in md_files:
        try:
            emojis_removed = remove_emojis_from_file(md_file)
            if emojis_removed > 0:
                rel_path = md_file.relative_to(project_root)
                print(f" {rel_path}: {emojis_removed} emojis removed")
                md_emojis_removed += emojis_removed
        except Exception as e:
            print(f" ERROR: {md_file}: {e}")

    print(f"Markdown files: {md_emojis_removed} emojis removed")

    total_all_emojis = total_emojis_removed + md_emojis_removed
    print()
