...
def main():
    ...
    if not success:
        print(
            "⚠️ Could not verify local app is running. Is it started in another terminal?"
        )
    ...
    if run_command("grep -r 'src=\"/' static/ | grep -v '/static/' | grep -q html"):
        ...
        print(
            '   These should be changed to relative paths (e.g., src="./path" instead of src="/path").'
        )
        fix = input("Attempt to automatically fix these issues? (y/n): ")
        if fix.lower() == "y":
            run_command(
                "find static -name '*.html' -type f -exec sed -i 's/src=\"\\//src=\".\\//' {} \\;"
            )
            run_command(
                "find static -name '*.html' -type f -exec sed -i 's/href=\"\\//href=\".\\//' {} \\;"
            )
    ...
    print(
        "3. Visit your deployed app at: https://<your-repl-name>.<username>.replit.app"
    )
    print(
        "4. Test the deployment using: https://<your-repl-name>.<username>.replit.app/deployment-test"
    )
