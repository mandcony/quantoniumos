...
def test_encryption_decryption_cycle(iterations: int = 5):
    ...
    for i in range(iterations):
        ...
        # Encrypt
        encrypt_payload = {"plaintext": plaintext, "key": key}
        encrypt_response = requests.post(
            f"{BASE_URL}/api/encrypt", headers=headers, json=encrypt_payload
        )

        if encrypt_response.status_code != 200:
            print(
                f"Iteration {i+1}: Encryption failed with status {encrypt_response.status_code}"
            )
            continue
        ...
        # Decrypt
        decrypt_payload = {"ciphertext": ciphertext, "key": key}
        decrypt_response = requests.post(
            f"{BASE_URL}/api/decrypt", headers=headers, json=decrypt_payload
        )

        if decrypt_response.status_code != 200:
            print(
                f"Iteration {i+1}: Decryption failed with status {decrypt_response.status_code}"
            )
            continue
        ...
        if decrypted_text == plaintext:
            success_count += 1
            print(
                f"Iteration {i+1}: ✅ Success - Round trip encryption/decryption works"
            )
        else:
            print(
                f"Iteration {i+1}: ❌ Failure - Decrypted text does not match original"
            )
            print(
                f"  Original: {plaintext[:30]}..."
                if len(plaintext) > 30
                else f"  Original: {plaintext}"
            )
            print(
                f"  Decrypted: {decrypted_text[:30]}..."
                if len(decrypted_text) > 30
                else f"  Decrypted: {decrypted_text}"
            )
...
def test_randomized_rft(iterations: int = 5):
    ...
    for i in range(iterations):
        ...
        response = requests.post(
            f"{BASE_URL}/api/simulate/rft", headers=headers, json=payload
        )
        ...
        if response.status_code == 200:
            ...
            print(
                f"Iteration {i+1}: ❌ Failure - RFT failed with status {response.status_code}"
            )
...
def test_randomized_entropy(iterations: int = 5):
    ...
    for i in range(iterations):
        ...
        response = requests.post(
            f"{BASE_URL}/api/entropy/sample", headers=headers, json=payload
        )

        if response.status_code == 200:
            entropy = response.json().get("entropy", "")
            print(
                f"Iteration {i+1}: ✅ Success - Generated {len(entropy)} chars of entropy for {amount} bytes request"
            )
            ...
            print(
                f"Iteration {i+1}: ❌ Failure - Entropy generation failed with status {response.status_code}"
            )
...
def test_randomized_container(iterations: int = 5):
    ...
    for i in range(iterations):
        ...
        # Call container API
        payload = {"waveform": waveform, "hash": random_hash}
        response = requests.post(
            f"{BASE_URL}/api/container/unlock", headers=headers, json=payload
        )
        ...
        if response.status_code == 200:
            ...
            if result:
                print(
                    f"Iteration {i+1}: ✅ Unexpected Success - Container unlocked (rare with random data)"
                )
            else:
                print(
                    f"Iteration {i+1}: ✓ Expected Failure - Container remained locked"
                )
        else:
            print(
                f"Iteration {i+1}: ❌ API Failure - Container API failed with status {response.status_code}"
            )
...
def full_randomized_test():
    ...
    if response.status_code == 200:
        ...
        if alt_response.status_code == 200:
            ...
            print(
                f"❌ API Status Check (alternate endpoint): Not responding properly (Status {alt_response.status_code})"
            )
    ...
    print(
        "This demonstrates architecture functionality using random data rather than exposing any real user inputs to the system."
    )
