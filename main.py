...
from auth import APIKey, APIKeyAuditLog, db, initialize_auth
...
from utils.security_logger import (SecurityEventType, SecurityOutcome,
                                   log_security_event, setup_security_logger)
...
def create_app():
    ...
    if not database_url:
        logger.warning(
            "DATABASE_URL not found. Falling back to SQLite in-memory database for development."
        )
