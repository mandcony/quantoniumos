...
class Oscillator(GeometricContainer):
    def __init__(self, frequency: float, amplitude: complex, phase: float):
        ...
        # Single vertex initialization for oscillator
    ...
    def update(self, dt: float):
        ...
        sample = self.amplitude * np.exp(
            1j * (2 * math.pi * self.frequency * t + self.phase)
        )
...
def validate_oscillator(osc: Oscillator, duration: float, dt: float = 0.1) -> list:
    ...
    if not (
        target_norm * (1 - tolerance) <= total_norm <= target_norm * (1 + tolerance)
    ):
        scale = target_norm / total_norm
        osc.waveform = [s * scale for s in osc.waveform]
        print(
            f"Waveform normalized: Total norm was {total_norm:.2f}, scaled to {target_norm}"
        )
