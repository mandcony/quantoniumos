...
try:
    ...
    HAS_QUBIT_STATE = False
    # Create a dummy MultiQubitState class
    class MultiQubitState:
        def __init__(self, num_qubits):
            self.num_qubits = num_qubits
        
        def measure_all(self):
            return np.random.randint(0, 2**self.num_qubits)
...
class GeometricContainer:
    def __init__(self, id, vertices, transformations=[], material_props={}):
        ...
        self.q_state = MultiQubitState(3)  # For tetrahedral symbolic mapping (memory mapper)
    ...
    def rotate(self, angles):
        ...
        if HAS_SCIPY:
            ...
            # Fallback: simple rotation around z-axis only
            angle_z = angles.get("z", 0)
    ...
    def map_symbolic_data(self, symbolic_payload):
        """
        Maps a symbolic payload (e.g., a list of characters or tokens) onto the tetrahedral geometry
        of the container. Cycles through the vertices, assigning each symbol and then computes
    ...
    def get_geometric_signature(self):
        ...
        return {
            ...
            "num_vertices": len(self.vertices),
            "center": center,
            "geometric_hash": self.symbolic_hash,
