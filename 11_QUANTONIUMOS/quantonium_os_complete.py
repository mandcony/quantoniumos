...
try:
    if web_deps_available:
        from app import app, socketio, initialize_quantum_system
        web_available = True
...
""")
    ...
    def show_system_info(self):
        ...
        print(f"   • Patent Integration: {'✅ Active' if self.integration else '❌ Inactive'}")
        print(f"   • Filesystem: {'✅ Mounted' if self.filesystem else '❌ Unmounted'}")
        print(f"   • Desktop GUI: {'✅ Available' if desktop_available else '❌ Unavailable'}")
        print(f"   • Web Interface: {'✅ Available' if web_available else '❌ Unavailable'}")
        
        if self.filesystem:
            stats = self.filesystem.get_filesystem_stats()
            print(f"📁 Filesystem: {stats['total_files']} files, {stats['total_size_bytes']} bytes")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="QuantoniumOS - 1000-Qubit Quantum Operating System")
    ...
    parser.add_argument('--no-kernel', action='store_true', help='Skip kernel initialization')
