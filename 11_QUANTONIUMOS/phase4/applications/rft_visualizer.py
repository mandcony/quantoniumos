...
class RFTTransformVisualizer:
    ...
    def setup_ui(self):
        ...
        ttk.Button(input_frame, text="Load File", command=self.load_data_file).pack(side=tk.LEFT, padx=(10, 5))
        ttk.Button(input_frame, text="Generate Synthetic", command=self.generate_synthetic_data).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(input_frame, text="Clear", command=self.clear_data).pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Button(transform_frame, text="Compute RFT", command=self.compute_rft).pack(side=tk.LEFT, padx=(10, 5))
        ttk.Button(transform_frame, text="Inverse RFT", command=self.compute_inverse_rft).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(transform_frame, text="Compare FFT", command=self.compare_with_fft).pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Checkbutton(viz_frame, text="Magnitude", variable=self.show_magnitude_var, 
                       command=self.update_visualization).pack(side=tk.LEFT, padx=(10, 5))
        
        self.show_phase_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(viz_frame, text="Phase", variable=self.show_phase_var,
                       command=self.update_visualization).pack(side=tk.LEFT, padx=(0, 5))
        
        self.show_real_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(viz_frame, text="Real", variable=self.show_real_var,
                       command=self.update_visualization).pack(side=tk.LEFT, padx=(0, 5))
        
        self.show_imaginary_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(viz_frame, text="Imaginary", variable=self.show_imaginary_var,
        ...
        ttk.Checkbutton(scale_frame, text="Log Scale", variable=self.log_scale_var,
                       command=self.update_visualization).pack(side=tk.LEFT, padx=(10, 5))
        
        self.normalize_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(scale_frame, text="Normalize", variable=self.normalize_var,
                       command=self.update_visualization).pack(side=tk.LEFT, padx=(0, 5))
        
        # Colormap selection
        ttk.Label(scale_frame, text="Colormap:").pack(side=tk.LEFT, padx=(20, 5))
        self.colormap_var = tk.StringVar(value="viridis")
        colormap_combo = ttk.Combobox(scale_frame, textvariable=self.colormap_var, 
        ...
        ttk.Checkbutton(realtime_frame, text="Auto Update", variable=self.auto_update_var,
                       command=self.toggle_auto_update).pack(side=tk.LEFT)
        
        ttk.Label(realtime_frame, text="Interval (ms):").pack(side=tk.LEFT, padx=(20, 5))
        self.interval_var = tk.StringVar(value="100")
        interval_entry = ttk.Entry(realtime_frame, textvariable=self.interval_var, width=8)
    ...
    def setup_subplots(self):
        ...
        # Style subplots
        for ax in [self.ax_input, self.ax_magnitude, self.ax_phase, self.ax_real, self.ax_imaginary]:
    ...
    def generate_synthetic_data(self):
        ...
        try:
            ...
            data_type_combo = ttk.Combobox(dialog, textvariable=data_type_var,
                                          values=["sine_wave", "chirp", "gaussian_pulse", "noise", "complex_signal"],
                                          state="readonly")
            ...
            def generate():
                try:
                    ...
                    if data_type == "sine_wave":
                        ...
                        signal = np.exp(-(t - n_samples/(2*fs))**2 / (2 * (0.1)**2)) * np.sin(2 * np.pi * frequency * t)
                    elif data_type == "noise":
                        signal = np.random.randn(n_samples)
                    elif data_type == "complex_signal":
                        signal = np.sin(2 * np.pi * frequency * t) + 1j * np.cos(2 * np.pi * frequency * t)
                    else:
                        signal = np.sin(2 * np.pi * frequency * t)
    ...
    def compute_inverse_rft(self):
        ...
        try:
            ...
            if self.input_data is not None:
                self.ax_input.plot(np.real(self.input_data), 'b-', label='Original', alpha=0.7)
            self.ax_input.plot(np.real(inverse_result), 'r--', label='Reconstructed', alpha=0.7)
...
Correlation: {np.corrcoef(np.abs(self.rft_result), np.abs(fft_result))[0,1]:.4f}
    ...
    def plot_input_data(self):
        ...
        if np.iscomplexobj(self.input_data):
            ...
            self.ax_input.plot(np.imag(self.input_data), 'r-', label='Imaginary', alpha=0.7)
            self.ax_input.legend()
        else:
    ...
    def plot_3d_visualization(self):
        ...
        surf = self.ax_3d.plot_surface(X, Y, Z, cmap=self.visualization_params['colormap'], alpha=0.8)
