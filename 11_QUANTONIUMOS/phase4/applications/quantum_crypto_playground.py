...
class QuantumCryptographyPlayground:
    ...
    def setup_protocol_tab(self):
        ...
        # Protocol selection
        selection_frame = ttk.LabelFrame(protocol_frame, text="Protocol Selection", padding=10)
        selection_frame.pack(fill=tk.X, pady=(0, 10))
        
        ttk.Label(selection_frame, text="Select Protocol:").pack(side=tk.LEFT)
        self.protocol_var = tk.StringVar(value="BB84")
        protocol_combo = ttk.Combobox(selection_frame, textvariable=self.protocol_var,
                                     values=list(self.protocols.keys()), state="readonly", width=20)
        ...
        ttk.Entry(key_frame, textvariable=self.key_length_var, width=10).pack(side=tk.LEFT, padx=(10, 0))
        ...
        ttk.Entry(error_frame, textvariable=self.error_rate_var, width=10).pack(side=tk.LEFT, padx=(10, 0))
        
        # Eavesdropper
        ttk.Checkbutton(params_frame, text="Eavesdropper Present", 
                       variable=tk.BooleanVar()).pack(anchor=tk.W, pady=(0, 5))
        ...
        noise_combo = ttk.Combobox(noise_frame, textvariable=self.noise_model_var,
                                  values=["Depolarizing", "Amplitude Damping", "Phase Damping", "Bit Flip"],
                                  state="readonly", width=15)
        noise_combo.pack(side=tk.LEFT, padx=(10, 0))
        
        # Protocol description
        desc_frame = ttk.LabelFrame(protocol_frame, text="Protocol Description", padding=10)
        desc_frame.pack(fill=tk.BOTH, expand=True)
        
        self.protocol_description = scrolledtext.ScrolledText(desc_frame, height=10, 
                                                             bg='#2e2e2e', fg='white',
    ...
    def setup_simulation_tab(self):
        ...
        ttk.Button(button_frame, text="Run Single", command=self.run_single_simulation).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Run Batch", command=self.run_batch_simulation).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Real-time", command=self.toggle_realtime_simulation).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(button_frame, text="Stop", command=self.stop_simulation).pack(side=tk.LEFT, padx=(0, 10))
        ...
        ttk.Label(progress_frame, textvariable=self.status_var).pack(side=tk.LEFT, padx=(20, 0))
        ...
        # Results tree
        self.results_tree = ttk.Treeview(results_frame, columns=('Protocol', 'Status', 'Key Rate', 'Error Rate', 'Time'),
    ...
    def setup_analysis_tab(self):
        ...
        ttk.Button(analysis_buttons, text="Security Analysis", command=self.run_security_analysis).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(analysis_buttons, text="Performance Analysis", command=self.run_performance_analysis).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(analysis_buttons, text="Error Analysis", command=self.run_error_analysis).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(analysis_buttons, text="Export Results", command=self.export_results).pack(side=tk.LEFT, padx=(0, 10))
    ...
    def setup_education_tab(self):
        ...
        concepts_text = scrolledtext.ScrolledText(concepts_frame, bg='#2e2e2e', fg='white',
                                                 insertbackground='white')
        ...
        tutorials_text = scrolledtext.ScrolledText(tutorials_frame, bg='#2e2e2e', fg='white',
                                                  insertbackground='white')
        ...
        examples_text = scrolledtext.ScrolledText(examples_frame, bg='#2e2e2e', fg='white',
                                                 insertbackground='white')
    ...
    def setup_tools_tab(self):
        ...
        # Tools selection
        tools_selection = ttk.LabelFrame(tools_frame, text="Cryptographic Tools", padding=10)
        tools_selection.pack(fill=tk.X, pady=(0, 10))
        
        tools_buttons = ttk.Frame(tools_selection)
        tools_buttons.pack(fill=tk.X)
        
        ttk.Button(tools_buttons, text="Random Number Generator", command=self.open_rng_tool).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(tools_buttons, text="Hash Calculator", command=self.open_hash_tool).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(tools_buttons, text="Key Generator", command=self.open_key_gen_tool).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(tools_buttons, text="Entropy Analyzer", command=self.open_entropy_tool).pack(side=tk.LEFT, padx=(0, 5))
...
Security: Enhanced security against realistic attack scenarios.
    ...
    def run_single_simulation(self):
        ...
        try:
            ...
            threading.Thread(target=self._run_simulation_thread, args=(protocol,), daemon=True).start()
    ...
    def run_batch_simulation(self):
        ...
        try:
            ...
            threading.Thread(target=self._run_batch_thread, args=(protocols,), daemon=True).start()
    ...
    def _run_simulation_thread(self, protocol):
        ...
        try:
            ...
            if protocol in self.protocols:
                ...
                self.root.after(0, self._add_result_to_tree, run_id, protocol, result, execution_time)
                ...
                self.root.after(0, lambda: self.status_var.set(f"{protocol} simulation completed"))
            else:
                self.root.after(0, lambda: self.status_var.set(f"Unknown protocol: {protocol}"))
        ...
        try:
            ...
            for i, protocol in enumerate(protocols):
                if protocol in self.protocols:
                    ...
                    run_id = f"Batch_{i+1}"
                    self.root.after(0, self._add_result_to_tree, run_id, protocol, result, 0)
                    
                # Update progress
                progress = (i + 1) / total * 100
                self.root.after(0, lambda p=progress: self.progress_var.set(p))
            
            self.root.after(0, lambda: self.status_var.set("Batch simulation completed"))
            
        except Exception as e:
            self.root.after(0, lambda: self.status_var.set(f"Batch simulation failed: {e}"))
    
    def _add_result_to_tree(self, run_id, protocol, result, execution_time):
        """Add simulation result to tree view"""
        self.results_tree.insert('', tk.END, text=run_id, values=(
            protocol,
    ...
    def simulate_b92(self):
        ...
        alice_final = [alice_bits[i] for i in range(len(alice_bits)) if bob_detections[i]]
    ...
    def simulate_e91(self):
        ...
        for _ in range(n_pairs):
            ...
            alice_basis = np.random.randint(0, 3)  # 0, 1, 2 for three measurement angles
        ...
        key_bits_alice = [alice_results[i] for i in range(len(alice_results)) if alice_bases[i] == bob_bases[i]]
        key_bits_bob = [bob_results[i] for i in range(len(bob_results)) if alice_bases[i] == bob_bases[i]]
    ...
    def run_security_analysis(self):
        ...
        analysis_text = scrolledtext.ScrolledText(analysis_window, bg='#2e2e2e', fg='white',
                                                 insertbackground='white')
        ...
        for data in security_data:
            ...
            analysis_report += f"  Status: {'SECURE' if data['secure'] else 'INSECURE'}\n\n"
    ...
    def run_performance_analysis(self):
        ...
        messagebox.showinfo("Info", "Performance analysis visualization not yet implemented")
