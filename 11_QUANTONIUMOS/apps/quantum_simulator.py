    ...
    def run_circuit(self):
        ...
        if not self.circuit:
            self.simulation_output.setPlainText(
                "No circuit to run. Add some gates first."
            )
            return
...
"""
    ...
    def check_entanglement(self):
        ...
        entangled = (
            n_qubits > 1
            and len(self.circuit) > 0
            and any("CNOT" in gate or "CZ" in gate for gate in self.circuit)
        )
...
"""
    ...
    def run_quantum_algorithm(self):
        ...
        if algorithm == "Deutsch-Jozsa Algorithm":
            ...
            self.algorithm_output.setPlainText(
                f"Algorithm '{algorithm}' implementation coming soon!"
            )
