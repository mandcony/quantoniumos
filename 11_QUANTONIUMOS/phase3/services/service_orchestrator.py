...
class ServiceOrchestrator:
    ...
    def register_service(self, config: ServiceConfiguration, handler: Callable) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Registered service: {config.name} (type: {config.service_type.value})")
            ...
            for dep in config.dependencies:
                if dep.dependency_type == "hard":
                    if not await self.start_service(dep.service_name):
                        self.logger.error(f"Failed to start dependency {dep.service_name}")
                        instance.status = ServiceStatus.ERROR
                        instance.error_message = f"Dependency {dep.service_name} failed to start"
                        return False
    ...
    async def stop_service(self, service_name: str, force: bool = False) -> bool:
        ...
        try:
            ...
            try:
                stop_task = asyncio.create_task(handler('stop', instance.config.config_params))
    ...
    async def _health_check_service(self, service_name: str):
        ...
        if (instance.last_health_check and 
            (now - instance.last_health_check).total_seconds() < config.health_check_interval):
            return
        ...
        try:
            ...
            if health_result:
                ...
                if config.auto_restart and instance.restart_count < config.max_restart_attempts:
                    self.logger.warning(f"Service {service_name} unhealthy, attempting restart")
    ...
    def _validate_dependencies(self, service_name: str) -> bool:
        ...
        for dep_name in self.dependency_graph[service_name]:
            if dep_name not in self.services:
                self.logger.error(f"Dependency {dep_name} not registered for service {service_name}")
                return False
        return True
