...
def fix_line_breaks(content):
    ...
    content = re.sub(
        r"import ([a-zA-Z_][a-zA-Z0-9_]*) from", r"import \1\nfrom", content
    )
    content = re.sub(
        r"from ([a-zA-Z_][a-zA-Z0-9_.]*) import ([a-zA-Z_][a-zA-Z0-9_]*) import",
        r"from \1 import \2\nimport",
        content,
    )
    ...
    content = re.sub(
        r"class ([a-zA-Z_][a-zA-Z0-9_]*): def", r"class \1:\n    def", content
    )
    content = re.sub(
        r"class ([a-zA-Z_][a-zA-Z0-9_]*)\([^)]+\): def",
        r"class \1(\2):\n    def",
        content,
    )
    ...
    content = re.sub(
        r"def ([a-zA-Z_][a-zA-Z0-9_]*)\([^)]*\): [a-zA-Z]",
        r"def \1(\2):\n        \3",
        content,
    )
    ...
    content = re.sub(
        r"while ([^:]+): ([a-zA-Z])", r"while \1:\n            \2", content
    )

    # Fix try/except
    content = re.sub(r"try: ([a-zA-Z])", r"try:\n            \1", content)
    content = re.sub(
        r"except ([^:]+): ([a-zA-Z])", r"except \1:\n            \2", content
    )
...
def fix_file(filepath):
    ...
    try:
        ...
        if char_count > 500 and line_count < 10:
            print(
                f"Fixing {filepath} (compressed: {line_count} lines, {char_count} chars)"
            )
