...
def check_dependencies():
    ...
    if required:
        ...
        print(
            "Ubuntu/Debian: sudo apt-get install cmake build-essential libeigen3-dev python3-pybind11"
        )
...
def build_with_cmake():
    ...
    build_result = subprocess.run(
        ["cmake", "--build", ".", "--config", "Release"], capture_output=True, text=True
    )
...
def test_engines():
    ...
    working_engines = sum(
        1 for result in test_results.values() if result.startswith("✓")
    )
...
def main():
    ...
    if test_engines():
        ...
        print("\nNext steps:")
        print(
            '1. Run: python -c "from core.high_performance_engine import get_engine_status; print(get_engine_status())"'
        )
        print(
            "2. Your Python code will now automatically use high-performance C++ implementations"
        )
    else:
