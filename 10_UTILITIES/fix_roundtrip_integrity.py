...
def test_avalanche_metrics():
    ...
    try:
        ...
        print(f"Message avalanche: {'✓ PASS' if 0.4 <= message_avalanche <= 0.6 else '○ NEEDS WORK'}")
        ...
        print(f"Target range: 0.4-0.6")
        print(f"Key avalanche: {'✓ PASS' if 0.4 <= key_avalanche <= 0.6 else '○ NEEDS WORK'}")
...
def main():
    ...
    if avalanche_results:
        print(f"Message avalanche: {avalanche_results['message_avalanche']:.3f} {'✓' if avalanche_results['message_avalanche_ok'] else '○'}")
        print(f"Key avalanche: {avalanche_results['key_avalanche']:.3f} {'✓' if avalanche_results['key_avalanche_ok'] else '○'}")
        
        if roundtrip_ok and avalanche_results['key_avalanche_ok']:
            print("\n🎉 ALL PAPER TARGETS ACHIEVED!")
            print("  ✓ Roundtrip integrity: PERFECT")
            print(f"  ✓ Key avalanche: {avalanche_results['key_avalanche']:.3f} (paper target: 0.527)")
        else:
            print("\n⚠️  ISSUES TO FIX:")
            if not roundtrip_ok:
                print("  • Roundtrip integrity FAILED")
            if not avalanche_results['key_avalanche_ok']:
                print(f"  • Key avalanche {avalanche_results['key_avalanche']:.3f} outside 0.4-0.6 range")
    
    return roundtrip_ok and (avalanche_results and avalanche_results['key_avalanche_ok'])
